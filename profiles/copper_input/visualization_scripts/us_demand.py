...
def render_plot(df, type):
    ...
    if type == 'By Scenario':
        plot_info = plot_settings['US Demand']['By Scenario']
        return plot_us_demand(df, plot_info['title'], plot_info['x_label'], plot_info['y_label'], type='By Scenario')
    else:
        plot_info = plot_settings['US Demand']['By Region']
        return plot_us_demand(df, plot_info['title'], plot_info['x_label'], plot_info['y_label'], type='By Region')


def plot_us_demand(df, title='Dispatched Electricity', x_axis_label='Time',
        ...
        for scenario in scenarios:
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    showlegend=True,
                    name=scenario,
                    hovertemplate=f'Scenario: {scenario} <br> Region: {region}<br>' + 'Time: %{x}<br>US Demand: %{y:.2f} TWh<br>'
                )
            )
    else:
