...
def process(dbs: dict):
    ...
    for scenario_name, db in dbs.items():
        df = db.copy()
        prov_df = df[df.variable.str.startswith("Transmission|")]
        # remove Transmission| infront of variables
        prov_df['variable'] = prov_df['variable'].str.replace('Transmission|', '')
        prov_df['value'] = prov_df['value'].astype(float)
        for region, short in utils.province_short.items():
            prov_df['region'] = prov_df['region'].str.replace(region, short)

        prov_df['region'] = prov_df['region'].str.replace('->', '<br>-><br>')



        transmissions.append(prov_df)

    full_net_new_cap = pd.concat(transmissions)
