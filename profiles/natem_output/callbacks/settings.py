...
def link(app):
    @app.callback(
    Output('natem-settings-upload-yaml-output', 'children'),
    Input('natem-settings-upload-yaml', 'contents'),
    ...
    def update_settings(content, filename):
        ...
        data_handler.profiles['NATEM Canada Output'].settings = settings
        return html.Div([
    ...
    @app.callback(
        Output('natem-technology-settings-output', 'children'),
    ...
    @app.callback(
        Output('natem-plot-settings-output', 'children'),
    ...
    @app.callback(
        Output({'type': 'natem-tech-update', 'index': ALL}, 'disabled'),
        Input({'type': 'natem-tech-update', 'index': ALL}, 'n_clicks'),
        Input({'type': 'natem-tech-name', 'index': ALL}, 'value'),
        Input({'type': 'natem-tech-group', 'index': ALL}, 'value'),
        Input({'type': 'natem-tech-color', 'index': ALL}, 'value'),
        Input({'type': 'natem-tech-group-color', 'index': ALL}, 'value'),
        State({'type': 'natem-tech-update', 'index': ALL}, 'disabled'),
    ...
    def tech_update(n_clicks, names, groups, colors, group_colors, disabled):
        ...
        if trigger_id['type'] == 'natem-tech-update':
            disabled[idx] = True
    ...
    @app.callback(
        Output({'type': 'natem-plot-update', 'index': ALL, 'subtype':ALL}, 'disabled'),
        Input({'type': 'natem-plot-update', 'index': ALL, 'subtype':ALL}, 'n_clicks'),
        Input({'type': 'natem-plot-title', 'index': ALL, 'subtype':ALL}, 'value'),
        Input({'type': 'natem-plot-x-axis', 'index': ALL, 'subtype':ALL}, 'value'),
        Input({'type': 'natem-plot-y-axis', 'index': ALL, 'subtype':ALL}, 'value'),
        State({'type': 'natem-plot-update', 'index': ALL, 'subtype':ALL}, 'disabled'),
        prevent_initial_call=True
    )
    def plot_update(n_clicks, titles, x_axis, y_axis, disabled):
        ...
        for i, out in enumerate(ctx.outputs_list):
            if out['id']['index'] == trigger_id['index']:
                if trigger_id['type'] == 'natem-plot-update':
                    disabled[idx] = True
