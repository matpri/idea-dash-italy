...
def link(app):
    @app.callback(
        ...
        Output({
            'type': 'natem-dispatch-year-select',
            'index': ALL
        }, 'data'),
        Output({
            'type': 'natem-dispatch-year-select',
        ...
        Output({
            'type': 'natem-dispatch-day-select',
            'index': ALL
        }, 'data'),
        Output({
            'type': 'natem-dispatch-day-select',
        ...
        Input({
            'type': 'natem-dispatch-plot-select',
            'index': ALL
        }, 'value'),
        Input({
            'type': 'natem-dispatch-aggregate-switch',
            'index': ALL
        }, 'checked'),
        Input({
            'type': 'natem-dispatch-scenario-multi-select',
            'index': ALL
        }, 'value'),
        Input({
            'type': 'natem-dispatch-region-select',
        ...
        Input({
            'type': 'natem-dispatch-download-button',
            'index': ALL
        }, 'n_clicks'),
        State({
            'type': 'natem-dispatch-region-select',
    ...
    def dispatch_callback(plot_select, aggregate_switch, scenario_multi_select, region_select, year_select, day_select,
        ...
        if 'natem-dispatch-download-button' in trigger_id['type']:
            idx = 0
            for i, id in enumerate(ctx.inputs_list[0]):
                if ((id['id']['index'] == trigger_id['index']) and
                        (id['id']['type'] == 'natem-dispatch-download-button')):
                    idx = i
                    break
            download[idx] = dcc.send_data_frame(data_handler.processed_data['NATEM Canada Output']['Dispatch'].to_csv,
                                                "dispatch.csv")
            return figure, region_data, region_value, year_data, year_value, day_data, day_value, download

        if 'natem-dispatch-scenario-multi-select' in trigger_id['type']:
            df = data_handler.processed_data['NATEM Canada Output']['Dispatch']
            idx = 0
            for i, id in enumerate(ctx.inputs_list[0]):
                if ((id['id']['index'] == trigger_id['index']) and
                        (id['id']['type'] == 'natem-dispatch-scenario-multi-select')):
                    idx = i
                    break
            ...
            figure[idx] = render_plot(plot_select[idx], data_handler.processed_data['NATEM Canada Output']['Dispatch'],
        ...
        for i, id in enumerate(ctx.inputs_list[0]):
            if ((id['id']['index'] == trigger_id['index']) and
                    (id['id']['type'] == 'natem-dispatch-plot-select')):
                idx = i
                break
        figure[idx] = render_plot(plot_select[idx], data_handler.processed_data['NATEM Canada Output']['Dispatch'],
