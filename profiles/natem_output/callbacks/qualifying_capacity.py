...
from profiles.natem_output.visualization_scripts.qualifying_capacity import render_plot
...
def link(app):
    @app.callback(
        ...
        Output({
            'type': 'natem-qualifying-capacity-year-select',
            'index': ALL
        }, 'style'),
        Output({
        ...
        Output({
            'type': 'natem-qualifying-capacity-scenario-select',
            'index': ALL
        }, 'style'),
        Output({
            'type': 'natem-qualifying-capacity-scenario-multi-select',
            'index': ALL
        }, 'style'),
        Output(
            {
                'type': 'natem-qualifying-capacity-pattern-switch',
                'index': ALL
            },
            'style'
        ),
        Output(
            {
                'type': 'natem-qualifying-capacity-text-switch',
                'index': ALL
            },
            'style'
        ),
        Input({
            'type': 'natem-qualifying-capacity-plot-select',
            'index': ALL
        }, 'value'),
        Input({
            'type': 'natem-qualifying-capacity-aggregate-switch',
            'index': ALL
        }, 'checked'),
        Input({
            'type': 'natem-qualifying-capacity-scenario-multi-select',
        ...
        Input({
            'type': 'natem-qualifying-capacity-download-button',
            'index': ALL
        }, 'n_clicks'),
        Input({
            'type': 'natem-qualifying-capacity-season-select',
            'index': ALL
        }, 'value'),
        State({
            'type': 'natem-qualifying-capacity-region-select',
    ...
    def update_net_new_capacity(_p_type, _aggregates, _scenarios, _scenario, _regions, _years, _pattern, _text,_download, _seasons,
        ...
        if 'natem-qualifying-capacity-download-button' in trigger_id['type']:
            idx = 0
            for i, id in enumerate(ctx.inputs_list[0]):
                if ((id['id']['index'] == trigger_id['index']) and
                        (id['id']['type'] == 'natem-qualifying-capacity-download-button')):
                    idx = i
                    break
            _data[idx] = dcc.send_data_frame(data_handler.processed_data['NATEM Canada Output']['Capacity'].to_csv,
        ...
        for i, id in enumerate(ctx.inputs_list[0]):
            if ((id['id']['index'] == trigger_id['index']) and
                    (id['id']['type'] == 'natem-qualifying-capacity-plot-select')):
                idx = i
                break
        ...
        if _p_type[idx] == 'By Year':
            ...
            if _aggregates[idx] is not None:
                _canvas[idx] = render_plot('By Year',
                                           data_handler.processed_data['NATEM Canada Output']['Qualifying Capacity'],
                                           _aggregates[idx],
                                           _scenarios[idx],
                                           _regions[idx],
                                           _years[idx],
