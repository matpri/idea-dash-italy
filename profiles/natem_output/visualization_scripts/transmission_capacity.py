...
from profiles.natem_output.visualization_scripts.utils import bar_over_regions, bar_over_years, trend_over_years
...
def transmission_plot(df, scenario, year, title):
    ...
    with open('./profiles/natem_output/visualization_scripts/utils/canada.geojson') as f:
        canada = geojson.load(f)
    with open('./profiles/natem_output/visualization_scripts/utils/arrows.geojson') as f:
...
def plot(df, window_id):
    ...
    widget_layout = html.Div([
        ...
        dmc.Select(
            ...
            id={
                'type': 'natem-transmissioncapacity-scenario-group-select',
                'index': window_id,
            },
            style={'display': 'none'}
        ),
        ...
        dmc.Select(
            ...
            value=lines[0],
            id={
                'type': 'natem-transmissioncapacity-lines-select',
                'index': window_id
            },
            style={'display': 'none'}
        ),

        dmc.Button('Download Data', id={'type': 'natem-transmissioncapacity-download-button', 'index': window_id},
                   variant='light',
                   # center the button
                     style={'display': 'flex', 'justify-content': 'center', 'margin-top': '4px'}),
        dcc.Download(id={'type': 'natem-transmissioncapacity-download', 'index': window_id}),
