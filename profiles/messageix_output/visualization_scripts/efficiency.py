...
def render_plot(type, df, aggregate, scenarios, region, year, scenario, pattern_active=True, text_active=False,
    ...
    name = plot_settings['Efficiency']['name']
    unit = plot_settings['Efficiency']['unit']

    if type == 'By Year':
        plot_info = plot_settings['Efficiency']['By Year']
        ...
        plot_info = plot_settings['Efficiency']['Trend Over Years']
        ...
        plot_info = plot_settings['Efficiency']['Pie Chart']
        ...
        title = plot_settings['Efficiency']['Map']['title']
        return map_plot.plot_map(df, scenario, year, title, name, unit, variables)
    else:
        plot_info = plot_settings['Efficiency']['By Region']
...
def plot(df, window_id):
    ...
    parents = ['Efficiency|' + parent for parent in parents]
    for i in range(max_depth):
        ...
        map_plot_widgets.append(
            dmc.MultiSelect(
                ...
                value=[],
                id={
                    'type': 'messageix-efficiency-multi-level-select',
                    'index': window_id,
                    'level': i
                },
    ...
    widget_layout = html.Div([
        ...
        *map_plot_widgets,
        by_year_widgets,
        by_region_widgets,
        dmc.Button('Download Data', id={'type': 'messageix-efficiency-download-button', 'index': window_id},
                   variant='light',
                   # center the button
                   style={'display': 'flex', 'justify-content': 'center', 'margin-top': '4px'}),
        dcc.Download(id={'type': 'messageix-efficiency-download', 'index': window_id}),
    ])
