...
def render_plot(type, df, aggregate, scenarios, region, year, scenario, pattern_active=True, text_active=False,
    ...
    name = plot_settings['Final Energy']['name']
    unit = plot_settings['Final Energy']['unit']

    if type == 'By Year':
        plot_info = plot_settings['Final Energy']['By Year']
...
def plot(df, window_id):
    ...
    widget_layout = html.Div([
        ...
        *map_plot_widgets,
        by_year_widgets,
        by_region_widgets,
        dmc.Button('Download Data', id={'type': 'messageix-final_energy-download-button', 'index': window_id},
                   variant='light',
                   # center the button
                   style={'display': 'flex', 'justify-content': 'center', 'margin-top': '4px'}),
        dcc.Download(id={'type': 'messageix-final_energy-download', 'index': window_id}),
    ])
