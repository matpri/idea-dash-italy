...
def render_plot(type, df, aggregate, scenarios, region, year, scenario, pattern_active=True, text_active=False, variable=None):
    ...
    name = plot_settings['Lifetime']['name']
    unit = plot_settings['Lifetime']['unit']
    if type == 'By Year':
        plot_info = plot_settings['Lifetime']['By Year']
...
def plot(df, window_id):
    ...
    widget_layout = html.Div([
        ...
        by_year_widgets,
        by_region_widgets,
        dmc.Button('Download Data', id={'type': 'messageix-lifetime-download-button', 'index': window_id},
                   variant='light',
                   # center the button
                     style={'display': 'flex', 'justify-content': 'center', 'margin-top': '4px'}),
        dcc.Download(id={'type': 'messageix-lifetime-download', 'index': window_id}),
    ])
