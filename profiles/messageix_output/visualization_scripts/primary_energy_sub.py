...
def render_plot(type, df, aggregate, scenarios, region, year, scenario, pattern_active=True, text_active=False,
    ...
    name = plot_settings['Primary Energy (substitution method)']['name']
    unit = plot_settings['Primary Energy (substitution method)']['unit']

    if type == 'By Year':
        plot_info = plot_settings['Primary Energy (substitution method)']['By Year']
        ...
        plot_info = plot_settings['Primary Energy (substitution method)']['Trend Over Years']
        ...
        plot_info = plot_settings['Primary Energy (substitution method)']['Pie Chart']
        return pie_chart.plot(df, scenario, region, year, aggregate, plot_info['title'], plot_info['x_label'],
                              plot_info['y_label'])
    elif type == 'Map Plot':
        title = plot_settings['Primary Energy (substitution method)']['Map']['title']
        return map_plot.plot_map(df, scenario, year, title, name, unit, variables)
    else:
        plot_info = plot_settings['Primary Energy (substitution method)']['By Region']
...
def plot(df, window_id):
    ...
    parents = ['Primary Energy (substitution method)|' + parent for parent in parents]
    ...
    widget_layout = html.Div([
        ...
        *map_plot_widgets,
        by_year_widgets,
        by_region_widgets,
        dmc.Button('Download Data', id={'type': 'messageix-primary_energy_sub-download-button', 'index': window_id},
                   variant='light',
                   # center the button
                   style={'display': 'flex', 'justify-content': 'center', 'margin-top': '4px'}),
        dcc.Download(id={'type': 'messageix-primary_energy_sub-download', 'index': window_id}),
    ])
