...
def render_plot(type, df, aggregate, scenarios, region, year, scenario, pattern_active=True, text_active=False, variables=None):
    ...
    name = plot_settings['Cost']['name']
    unit = plot_settings['Cost']['unit']
    if type == 'By Year':
        plot_info = plot_settings['Cost']['By Year']
        ...
        title = plot_settings['Cost']['Map']['title']
        return map_plot.plot_map(df, scenario, year, title, name, unit, variables)
    else:
        plot_info = plot_settings['Cost']['By Region']
...
def plot(df, window_id):
    ...
    widget_layout = html.Div([
        ...
        by_year_widgets,
        by_region_widgets,
        dmc.Button('Download Data', id={'type': 'messageix-cost-download-button', 'index': window_id},
                   variant='light',
                   # center the button
                     style={'display': 'flex', 'justify-content': 'center', 'margin-top': '4px'}),
        dcc.Download(id={'type': 'messageix-cost-download', 'index': window_id}),
    ])
