...
def link(app):
    @app.callback(
        ...
        Input({
            'type': 'messageix-resource-aggregate-switch',
            'index': ALL
        }, 'checked'),
        Input({
            'type': 'messageix-resource-scenario-multi-select',
    ...
    def update_resource(_p_type, _aggregates, _scenarios, _scenario, _regions, _years, _variables, _pattern, _text,
        ...
        if 'messageix-resource-download-button' in trigger_id['type']:
            ...
            for i, id in enumerate(ctx.inputs_list[0]):
                if ((id['id']['index'] == trigger_id['index']) and
                        (id['id']['type'] == 'messageix-resource-download-button')):
                    idx = i
                    break
            _data[idx] = dcc.send_data_frame(data_handler.processed_data['MESSAGEix-Canada']['Resource'].to_csv, "resource.csv")
            return _canvas, _r_style, _y_style, _v_style, _v_data, _variables, _data, _s_style, _m_style, _pattern_style, _text_style
        idx = 0
        for i, id in enumerate(ctx.inputs_list[0]):
            if ((id['id']['index'] == trigger_id['index']) and
                    (id['id']['type'] == 'messageix-resource-plot-select')):
                idx = i
                break
        ...
        if _p_type[idx] == 'By Year':
            ...
            if _aggregates[idx] is not None:
                _canvas[idx] = render_plot('By Year', data_handler.processed_data['MESSAGEix-Canada']['Resource'],
                                           _aggregates[idx],
                                           _scenarios[idx],
                                           _regions[idx],
            ...
            if _aggregates[idx] is not None:
                _canvas[idx] = render_plot('Trend Over Years', data_handler.processed_data['MESSAGEix-Canada']['Resource'],
                                           _aggregates[idx],
                                           _scenarios[idx],
                                           _regions[idx],
            ...
            if _aggregates[idx] is not None:
                _canvas[idx] = render_plot('Pie Chart', data_handler.processed_data['MESSAGEix-Canada']['Resource'],
                                           _aggregates[idx],
                                           _scenarios[idx],
                                           _regions[idx],
                                           _years[idx], scenario=_scenario[idx], variables=_variables[idx])
            ...
            if 'messageix-resource-aggregate-switch' in trigger_id['type']:
                if _aggregates[idx] is not None:
            ...
            if _aggregates[idx] is not None:
                _canvas[idx] = render_plot('Map Plot', data_handler.processed_data['MESSAGEix-Canada']['Resource'],
                                           _aggregates[idx],
                                           _scenarios[idx],
                                           _regions[idx],
                                           _years[idx], scenario=_scenario[idx], variables=_variables[idx])
            ...
            if _aggregates[idx] is not None:
                _canvas[idx] = render_plot('By Region', data_handler.processed_data['MESSAGEix-Canada']['Resource'],
                                           _aggregates[idx],
                                           _scenarios[idx],
                                           _regions[idx],
                                           _years[idx], scenario=_scenario[idx],
