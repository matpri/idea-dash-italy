...
def link(app):
    @app.callback(
        ...
        Output({
            'type': 'messageix-total_cost-multi-level-select',
            'index': MATCH,
            'level': ALL
        }, 'style'),
        Output({
    ...
    def update_total_cost(_p_type, _show_sectors, _scenarios, _scenario, _regions, _years, _types, _levels, _pattern,
        ...
        if 'messageix-total_cost-type-select' in \
                trigger_id['type'] or 'messageix-total_cost-multi-level-select' in trigger_id['type'] or 'messageix-total_cost-show_sector-switch' in trigger_id['type']:
        ...
        if 'messageix-total_cost-multi-level-select' in trigger_id['type']:
