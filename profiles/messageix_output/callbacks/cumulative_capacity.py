...
from profiles.messageix_output.visualization_scripts.cumulative_capacity import render_plot
...
def link(app):
    @app.callback(
        ...
        Output({
            'type': 'messageix-cumulative_capacity-multi-level-select',
            'index': MATCH,
            'level': ALL
        }, 'style'),
        Output({
        ...
        Output({
            'type': 'messageix-cumulative_capacity-download',
            'index': MATCH
        }, 'data'),
        Output({
        ...
        Output({
            'type': 'messageix-cumulative_capacity-show_sector-switch',
            'index': MATCH
        }, 'label'),
        Output({
            'type': 'messageix-cumulative_capacity-type-select',
            'index': MATCH
        }, 'data'),
        Output({
            'type': 'messageix-cumulative_capacity-type-select',
        ...
        Input({
            'type': 'messageix-cumulative_capacity-download-button',
            'index': MATCH
        }, 'n_clicks'),
        State({
            'type': 'messageix-cumulative_capacity-region-select',
    ...
    def update_cumulative_capacity(_p_type, _show_sectors, _scenarios, _scenario, _regions, _years, _types, _levels, _pattern,
                            _text,
        ...
        if 'messageix-cumulative_capacity-download-button' in trigger_id['type']:

            _data = dcc.send_data_frame(data_handler.processed_data['MESSAGEix-Canada']['Cumulative Capacity'].to_csv,
        ...
        if 'messageix-cumulative_capacity-type-select' in \
                trigger_id['type'] or 'messageix-cumulative_capacity-multi-level-select' in trigger_id['type'] or 'messageix-cumulative_capacity-show_sector-switch' in trigger_id['type']:

            df_scen = df_scen[(df_scen['region'] == _regions) & (df_scen['time'] == _years) & (
                    df_scen['scenario'] == _scenario)]

            if 'messageix-cumulative_capacity-show_sector-switch' in trigger_id['type']:
                _types = 'All'
                _t_data = df_scen[df_scen['type']].type.unique().tolist()
                _t_data += ['All']
                _t_data = [{'label': x, 'value': x} for x in _t_data]
            if 'messageix-cumulative_capacity-type-select' in trigger_id['type'] or 'messageix-cumulative_capacity-show_sector-switch' in trigger_id['type']:
                if _types != 'All':
                ...
                _p = ['Cumulative Capacity|' + parent for parent in _p]
        ...
        if 'messageix-cumulative_capacity-multi-level-select' in trigger_id['type']:
        ...
        if _p_type== 'By Year':
            ...
            if _show_sectors is not None:
                _canvas= render_plot('By Year', data_handler.processed_data['MESSAGEix-Canada']['Cumulative Capacity'],
                                           _show_sectors,
                                           _scenarios,
                                           _regions,
            ...
            if _show_sectors is not None:
                _canvas = render_plot('Pie Chart', data_handler.processed_data['MESSAGEix-Canada']['Cumulative Capacity'],
                                           _show_sectors,
                                           _scenarios,
                                           _regions,
                                           _years, scenario=_scenario, variables=variables)
            ...
            if _show_sectors is not None:
                _canvas = render_plot('Map Plot', data_handler.processed_data['MESSAGEix-Canada']['Cumulative Capacity'],
                                           _show_sectors,
                                           _scenarios,
                                           _regions,
                                           _years, scenario=_scenario, variables=variables)
            ...
            if _show_sectors is not None:
                _canvas = render_plot('By Region', data_handler.processed_data['MESSAGEix-Canada']['Cumulative Capacity'],
                                           _show_sectors,
                                           _scenarios,
                                           _regions,
                                           _years, scenario=_scenario,
