...
from profiles.messageix_output.visualization_scripts.primary_energy_sub import render_plot
...
def link(app):
    @app.callback(
        Output({
            ...
            'viz': 'primary_energy_sub'
        }, 'figure'),
        Output({
            'type': 'messageix-primary_energy_sub-region-select',
            'index': MATCH
        }, 'style'),
        Output({
            'type': 'messageix-primary_energy_sub-year-select',
            'index': MATCH
        }, 'style'),
        Output({
            'type': 'messageix-primary_energy_sub-level-select',
            'index': MATCH,
            'level': ALL
        }, 'style'),
        Output({
        ...
        Output({
            'type': 'messageix-primary_energy_sub-download',
            'index': MATCH
        }, 'data'),
        Output({
            'type': 'messageix-primary_energy_sub-scenario-select',
            'index': MATCH
        }, 'style'),
        Output({
            'type': 'messageix-primary_energy_sub-scenario-multi-select',
            'index': MATCH
        }, 'style'),
        Output(
            {
                'type': 'messageix-primary_energy_sub-pattern-switch',
                'index': MATCH
            },
            'style'
        ),
        Output(
            {
                'type': 'messageix-primary_energy_sub-text-switch',
                'index': MATCH
            },
            'style'
        ),
        Output({
            'type': 'messageix-primary_energy_sub-show_sector-switch',
            'index': MATCH
        }, 'label'),
        Output({
            'type': 'messageix-primary_energy_sub-type-select',
            'index': MATCH
        }, 'data'),
        Output({
            'type': 'messageix-primary_energy_sub-type-select',
        ...
        Input({
            'type': 'messageix-primary_energy_sub-plot-select',
            'index': MATCH
        }, 'value'),
        Input({
            'type': 'messageix-primary_energy_sub-show_sector-switch',
        ...
        Input({
            'type': 'messageix-primary_energy_sub-download-button',
            'index': MATCH
        }, 'n_clicks'),
        State({
            'type': 'messageix-primary_energy_sub-region-select',
    ...
    def update_primary_energy_sub(_p_type, _show_sectors, _scenarios, _scenario, _regions, _years, _types, _levels, _pattern,
                            _text,
                            _download, _r_style, _y_style, _l_style, _l_data, _canvas, _data, _s_style, _m_style,
                            _pattern_style, _text_style):
        print('updating primary_energy_sub plot')
        ...
        if 'messageix-primary_energy_sub-download-button' in trigger_id['type']:

            _data = dcc.send_data_frame(data_handler.processed_data['MESSAGEix-Canada']['Primary Energy (substitution method)'].to_csv,
                                             "primary_energy_sub.csv")
        ...
        df_scen = data_handler.processed_data['MESSAGEix-Canada']['Primary Energy (substitution method)'].copy(deep=True)

        # df_scen = df_scen[df_scen['type'].isin(sectors)] if _show_sectors else df_scen[df_scen['type'].isin(sources)]

        if 'messageix-primary_energy_sub-type-select' in \
                trigger_id['type'] or 'messageix-primary_energy_sub-level-select' in trigger_id['type'] or 'messageix-primary_energy_sub-show_sector-switch' in trigger_id['type']:

            df_scen = df_scen[(df_scen['region'] == _regions) & (df_scen['time'] == _years) & (
                    df_scen['scenario'] == _scenario)]

            if 'messageix-primary_energy_sub-show_sector-switch' in trigger_id['type']:
                _types = 'All'
                _t_data = df_scen[df_scen['type']].type.unique().tolist()
                _t_data += ['All']
                _t_data = [{'label': x, 'value': x} for x in _t_data]
            if 'messageix-primary_energy_sub-type-select' in trigger_id['type'] or 'messageix-primary_energy_sub-show_sector-switch' in trigger_id['type']:
                if _types != 'All':
                ...
                _p = ['Primary Energy (substitution method)|' + parent for parent in _p]
        ...
        if 'messageix-primary_energy_sub-level-select' in trigger_id['type']:
        ...
        if _p_type== 'By Year':
            ...
            if _show_sectors is not None:
                _canvas= render_plot('By Year', data_handler.processed_data['MESSAGEix-Canada']['Primary Energy (substitution method)'],
                                           _show_sectors,
                                           _scenarios,
                                           _regions,
            ...
            if _show_sectors is not None:
                _canvas = render_plot('Trend Over Years',
                                           data_handler.processed_data['MESSAGEix-Canada']['Primary Energy (substitution method)'],
                                           _show_sectors,
                                           _scenarios,
                                           _regions,
            ...
            if _show_sectors is not None:
                _canvas = render_plot('Pie Chart', data_handler.processed_data['MESSAGEix-Canada']['Primary Energy (substitution method)'],
                                           _show_sectors,
                                           _scenarios,
                                           _regions,
                                           _years, scenario=_scenario, variables=variables)
            ...
            if _show_sectors is not None:
                _canvas = render_plot('Map Plot', data_handler.processed_data['MESSAGEix-Canada']['Primary Energy (substitution method)'],
                                           _show_sectors,
                                           _scenarios,
                                           _regions,
                                           _years, scenario=_scenario, variables=variables)
            ...
            if _show_sectors is not None:
                _canvas = render_plot('By Region', data_handler.processed_data['MESSAGEix-Canada']['Primary Energy (substitution method)'],
                                           _show_sectors,
                                           _scenarios,
                                           _regions,
                                           _years, scenario=_scenario,
