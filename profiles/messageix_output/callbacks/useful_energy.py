...
def link(app):
    @app.callback(
        ...
        Output({
            'type': 'messageix-useful_energy-show_sector-switch',
            'index': MATCH
        }, 'label'),
        Output({
            'type': 'messageix-useful_energy-type-select',
            'index': MATCH
        }, 'data'),
        Output({
            'type': 'messageix-useful_energy-type-select',
    ...
    def update_useful_energy(_p_type, _show_sectors, _scenarios, _scenario, _regions, _years, _types, _levels, _pattern,
                            _text,
        ...
        if 'messageix-useful_energy-download-button' in trigger_id['type']:
            ...
            _data = dcc.send_data_frame(data_handler.processed_data['MESSAGEix-Canada']['Useful Energy'].to_csv,
                                             "useful_energy.csv")
        ...
        df_scen = data_handler.processed_data['MESSAGEix-Canada']['Useful Energy'].copy(deep=True)

        # df_scen = df_scen[df_scen['type'].isin(sectors)] if _show_sectors else df_scen[df_scen['type'].isin(sources)]

        if 'messageix-useful_energy-type-select' in \
                trigger_id['type'] or 'messageix-useful_energy-multi-level-select' in trigger_id['type'] or 'messageix-useful_energy-show_sector-switch' in trigger_id['type']:

            df_scen = df_scen[(df_scen['region'] == _regions) & (df_scen['time'] == _years) & (
                    df_scen['scenario'] == _scenario)]

            if 'messageix-useful_energy-show_sector-switch' in trigger_id['type']:
                _types = 'All'
                _t_data = df_scen[df_scen['type']].type.unique().tolist()
                _t_data += ['All']
                _t_data = [{'label': x, 'value': x} for x in _t_data]
            if 'messageix-useful_energy-type-select' in trigger_id['type'] or 'messageix-useful_energy-show_sector-switch' in trigger_id['type']:
                if _types != 'All':
                ...
                _p = ['Useful Energy|' + parent for parent in _p]
        ...
        if 'messageix-useful_energy-multi-level-select' in trigger_id['type']:
        ...
        if _p_type== 'By Year':
            ...
            if _show_sectors is not None:
                _canvas= render_plot('By Year', data_handler.processed_data['MESSAGEix-Canada']['Useful Energy'],
                                           _show_sectors,
                                           _scenarios,
                                           _regions,
            ...
            if _show_sectors is not None:
                _canvas = render_plot('Trend Over Years by Variable',
                                           data_handler.processed_data['MESSAGEix-Canada']['Useful Energy'],
                                           _show_sectors,
                                           _scenarios,
                                           _regions,
            ...
            if _show_sectors is not None:
                _canvas = render_plot('Pie Chart', data_handler.processed_data['MESSAGEix-Canada']['Useful Energy'],
                                           _show_sectors,
                                           _scenarios,
                                           _regions,
                                           _years, scenario=_scenario, variables=variables)
            ...
            if _show_sectors is not None:
                _canvas = render_plot('Map Plot', data_handler.processed_data['MESSAGEix-Canada']['Useful Energy'],
                                           _show_sectors,
                                           _scenarios,
                                           _regions,
                                           _years, scenario=_scenario, variables=variables)
            ...
            if _show_sectors is not None:
                _canvas = render_plot('By Region', data_handler.processed_data['MESSAGEix-Canada']['Useful Energy'],
                                           _show_sectors,
                                           _scenarios,
                                           _regions,
                                           _years, scenario=_scenario,
