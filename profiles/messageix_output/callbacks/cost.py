...
from profiles.messageix_output.visualization_scripts.cost import render_plot
...
def link(app):
    @app.callback(
        ...
        Output({
            'type': 'messageix-cost-region-select',
            'index': MATCH
        }, 'style'),
        Output({
            'type': 'messageix-cost-year-select',
            'index': MATCH
        }, 'style'),
        Output({
        ...
        Output({
            'type': 'messageix-cost-download',
            'index': MATCH
        }, 'data'),
        Output({
            'type': 'messageix-cost-scenario-select',
            'index': MATCH
        }, 'style'),
        Output({
            'type': 'messageix-cost-scenario-multi-select',
            'index': MATCH
        }, 'style'),
        Output(
            {
                'type': 'messageix-cost-pattern-switch',
                'index': MATCH
            },
            'style'
        ),
        Output(
            {
                'type': 'messageix-cost-text-switch',
                'index': MATCH
            },
            'style'
        ),
        Output({
            'type': 'messageix-cost-show_sector-switch',
            'index': MATCH
        }, 'label'),
        Output({
            'type': 'messageix-cost-type-select',
            'index': MATCH
        }, 'data'),
        Output({
            'type': 'messageix-cost-type-select',
        ...
        Input({
            'type': 'messageix-cost-download-button',
            'index': MATCH
        }, 'n_clicks'),
        State({
            'type': 'messageix-cost-region-select',
    ...
    def update_cost(_p_type, _show_sectors, _scenarios, _scenario, _regions, _years, _types, _levels, _pattern,
                            _text,
                            _download, _r_style, _y_style, _l_style, _l_data, _canvas, _data, _s_style, _m_style,
                            _pattern_style, _text_style):
        print('updating cost plot')
        ...
        if 'messageix-cost-download-button' in trigger_id['type']:

            _data = dcc.send_data_frame(data_handler.processed_data['MESSAGEix-Canada']['Cost'].to_csv,
                                             "cost.csv")
        ...
        df_scen = data_handler.processed_data['MESSAGEix-Canada']['Cost'].copy(deep=True)

        # df_scen = df_scen[df_scen['type'].isin(sectors)] if _show_sectors else df_scen[df_scen['type'].isin(sources)]

        if 'messageix-cost-type-select' in \
                trigger_id['type'] or 'messageix-cost-multi-level-select' in trigger_id['type'] or 'messageix-cost-show_sector-switch' in trigger_id['type']:

            df_scen = df_scen[(df_scen['region'] == _regions) & (df_scen['time'] == _years) & (
                    df_scen['scenario'] == _scenario)]

            if 'messageix-cost-show_sector-switch' in trigger_id['type']:
                _types = 'All'
                _t_data = df_scen[df_scen['type']].type.unique().tolist()
                _t_data += ['All']
                _t_data = [{'label': x, 'value': x} for x in _t_data]
            if 'messageix-cost-type-select' in trigger_id['type'] or 'messageix-cost-show_sector-switch' in trigger_id['type']:
                if _types != 'All':
                ...
                _p = ['Cost|' + parent for parent in _p]
        ...
        if 'messageix-cost-multi-level-select' in trigger_id['type']:
        ...
        if _p_type== 'By Year':
            ...
            if _show_sectors is not None:
                _canvas= render_plot('By Year', data_handler.processed_data['MESSAGEix-Canada']['Cost'],
                                           _show_sectors,
                                           _scenarios,
                                           _regions,
            ...
            if _show_sectors is not None:
                _canvas = render_plot('Trend Over Years by Variable',
                                           data_handler.processed_data['MESSAGEix-Canada']['Cost'],
                                           _show_sectors,
                                           _scenarios,
                                           _regions,
            ...
            if _show_sectors is not None:
                _canvas = render_plot('Pie Chart', data_handler.processed_data['MESSAGEix-Canada']['Cost'],
                                           _show_sectors,
                                           _scenarios,
                                           _regions,
                                           _years, scenario=_scenario, variables=variables)
            ...
            if _show_sectors is not None:
                _canvas = render_plot('Map Plot', data_handler.processed_data['MESSAGEix-Canada']['Cost'],
                                           _show_sectors,
                                           _scenarios,
                                           _regions,
                                           _years, scenario=_scenario, variables=variables)
            ...
            if _show_sectors is not None:
                _canvas = render_plot('By Region', data_handler.processed_data['MESSAGEix-Canada']['Cost'],
                                           _show_sectors,
                                           _scenarios,
                                           _regions,
                                           _years, scenario=_scenario,
