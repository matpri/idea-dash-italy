...
def check(df):
    """
    Check if 'Primary Energy|' is present in the 'variable' column.
    ...
    """
    print("Checking for Primary Energy in variable column")
    try:
...
def process(selected: dict):
    ...
    for scenario_name, db in selected.items():
        df = db.copy()
        # filter where 'Primary Energy|' in variable column entry and remove the prefix
