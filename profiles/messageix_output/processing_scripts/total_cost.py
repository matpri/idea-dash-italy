...
def check(df):
    """
    Check if 'Total Cost|' is present in the 'variable' column.
    ...
    """
    print("Checking for Total Cost in variable column")
    try:
        if (df.model == 'MESSAGEix-Canada').any():
            if df.variable.str.startswith("Total Cost|").any():
                return True
        return False
    except Exception as e:
        print("Total Cost check", e)
        return False
...
def process(selected: dict):
    ...
    for scenario_name, db in selected.items():
        df = db.copy()
        # filter where 'Total Cost|' in variable column entry and remove the prefix
        df = df[df.variable.str.startswith("Total Cost|")]
