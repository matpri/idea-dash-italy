...
def check(df):
    """
    Check if 'Efficiency|' is present in the 'variable' column.
    ...
    """
    print("Checking for Efficiency in variable column")
    try:
...
def process(selected: dict):
    ...
    for scenario_name, db in selected.items():
        df = db.copy()
        # filter where 'Efficiency|' in variable column entry and remove the prefix
