...
def check(df):
    """
    Check if 'Cumulative Capacity|' is present in the 'variable' column.
    ...
    """
    print("Checking for Cumulative Capacity in variable column")
    try:
        if (df.model == 'MESSAGEix-Canada').any():
            if df.variable.str.startswith("Cumulative Capacity|").any():
                return True
        return False
    except Exception as e:
        print("Cumulative Capacity check", e)
        return False
...
def process(selected: dict):
    ...
    for scenario_name, db in selected.items():
        df = db.copy()
        # filter where 'Cumulative Capacity|' in variable column entry and remove the prefix
        df = df[df.variable.str.startswith("Cumulative Capacity|")]
    ...
    print("Cumulative Capacity processed")
    return full_df