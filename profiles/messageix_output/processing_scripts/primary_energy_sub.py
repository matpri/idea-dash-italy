...
def check(df):
    """
    Check if 'Primary Energy (substitution method)|' is present in the 'variable' column.
    ...
    """
    print("Checking for Primary Energy (substitution method) in variable column")
    try:
        if (df.model == 'MESSAGEix-Canada').any():
            if df.variable.str.startswith("Primary Energy (substitution method)|").any():
                return True
        return False
    except Exception as e:
        print("Primary Energy (substitution method) check", e)
        return False
...
def process(selected: dict):
    ...
    print("Primary Energy (substitution method) processed")
    return full_df