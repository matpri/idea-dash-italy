...
def check(df):
    """
    Check if 'Consumption|' is present in the 'variable' column.
    ...
    """
    print("Checking for Consumption in variable column")
    try:
        if (df.model == 'MESSAGEix-Canada').any():
            if df.variable.str.startswith("Consumption|").any():
                return True
        return False
    except Exception as e:
        print("Consumption check", e)
        return False
...
def process(selected: dict):
    ...
    for scenario_name, db in selected.items():
        df = db.copy()
        # filter where 'Consumption|' in variable column entry and remove the prefix
        df = df[df.variable.str.startswith("Consumption|")]
