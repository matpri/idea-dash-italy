...
def check(df):
    """
    Check if 'Capacity|' is present in the 'variable' column.
    ...
    """
    print("Checking for Capacity in variable column")
    try:
...
def process(selected: dict):
    ...
    for scenario_name, db in selected.items():
        df = db.copy()
        # filter where 'Capacity|' in variable column entry and remove the prefix
        df = df[df.variable.str.startswith("Capacity|")]
