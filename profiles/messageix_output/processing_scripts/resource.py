...
def check(df):
    """
    Check if 'Resource|' is present in the 'variable' column.
    ...
    """
    print("Checking for Resource in variable column")
    try:
...
def process(selected: dict):
    ...
    for scenario_name, db in selected.items():
        df = db.copy()
        # filter where 'Resource|' in variable column entry and remove the prefix
        df = df[df.variable.str.startswith("Resource|")]
