...
def check(df):
    """
    Check if 'Investment|' is present in the 'variable' column.
    ...
    """
    print("Checking for Investment in variable column")
    try:
        if (df.model == 'MESSAGEix-Canada').any():
            if df.variable.str.startswith("Investment|").any():
                return True
        return False
    except Exception as e:
        print("Investment check", e)
        return False
...
def process(selected: dict):
    ...
    for scenario_name, db in selected.items():
        df = db.copy()
        # filter where 'Investment|' in variable column entry and remove the prefix
        df = df[df.variable.str.startswith("Investment|")]
