...
def check(df):
    """
    Check if 'Useful Energy|' is present in the 'variable' column.
    ...
    """
    print("Checking for Useful Energy in variable column")
    try:
        if (df.model == 'MESSAGEix-Canada').any():
            if df.variable.str.startswith("Useful Energy|").any():
                return True
        return False
    except Exception as e:
        print("Useful Energy check", e)
        return False
...
def process(selected: dict):
    ...
    for scenario_name, db in selected.items():
        df = db.copy()
        # filter where 'Useful Energy|' in variable column entry and remove the prefix
