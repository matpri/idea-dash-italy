...
def link(app):
    ...
    def update_uc_emissions(_p_type, _scenarios, _scenario, _ts, _download, _canvas, _data, _s_style, _m_style):
        ...
        if 'silver-uc_emissions-download-button' in trigger_id['type']:
            ...
            for i, id in enumerate(ctx.inputs_list[0]):
                if ((id['id']['index'] == trigger_id['index']) and
                    ...
                    idx = i
                    break
            _data[idx] = dcc.send_data_frame(data_handler.processed_data['SILVER']['UC Emissions'].to_csv, "uc_emissions.csv")
        ...
        if _p_type[idx] == 'Total':
            ...
            _m_style[idx] = {'display': 'none'}
            _s_style[idx] = {'display': 'block'}
            _canvas[idx] = render_plot('By Technology', data_handler.processed_data['SILVER']['UC Emissions'], _scenario[idx], time_size=_ts[idx])
