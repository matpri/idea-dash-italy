...
from profiles.silver_output.visualization_scripts.uc_emissions import render_plot
...
def link(app):
    ...
    def update_uc_emissions(_p_type, _scenarios, _scenario, _ts, _download, _canvas, _data, _s_style, _m_style, _ts_style):
        print('updating uc_emissions plot')
        from utils.data_state import data_handler
        ctx = dash.callback_context
        trigger_id = eval(ctx.triggered[0]['prop_id'].split('.')[0])

        if 'silver-uc_emissions-download-button' in trigger_id['type']:
            idx = 0
            for i, id in enumerate(ctx.inputs_list[0]):
                if ((id['id']['index'] == trigger_id['index']) and
                        (id['id']['type'] == 'silver-uc_emissions-download-button')):
                    idx = i
                    break
            _data[idx] = dcc.send_data_frame(data_handler.processed_data['SILVER']['UC Emissions'].to_csv,
                                             "uc_emissions.csv")
        ...
        for i, id in enumerate(ctx.inputs_list[0]):
            if ((id['id']['index'] == trigger_id['index']) and
                    (id['id']['type'] == 'silver-uc_emissions-plot-select')):
                idx = i
                break
        ...
        if _p_type[idx] == 'Total':
            ...
            _canvas[idx] = render_plot('Total', data_handler.processed_data['SILVER']['UC Emissions'],
            ...
            _canvas[idx] = render_plot('By Plant',
                                       data_handler.processed_data['SILVER']['UC Emissions'],
            ...
            _s_style[idx] = {'display': 'block'}
            _ts_style[idx] = {'display': 'block'}
            _canvas[idx] = render_plot('By Technology', data_handler.processed_data['SILVER']['UC Emissions'],
                                       _scenario[idx], time_size=_ts[idx])
