...
def db_check(df):
    ...
    print("Checking for UC_Results in variable column")
    try:
        classes = df[df['model'] == 'silver']["variable"].apply(lambda x: x.split("|")[0])
        if (classes == 'UC_VRE_Curtailment').any():
            return True
        return False
...
def check(df):
    ...
    try:
        if type(df) is pd.DataFrame:
            ...
            if any(c.startswith('UC_VRE_Curtailment') for c in classes):
                return True
...
def aggregate_db(db, scenario):
    ...
    df = db[db['model']=='silver'][classes == 'UC_VRE_Curtailment']
...
def process(selected):
    ...
    for scenario, db in selected.items():
        if type(db) is pd.DataFrame:
            ...
            uc_vre_var = [k for k in db['data'].keys() if k.startswith('UC_VRE_Curtailment')][0]
            gens = db['data']['generator']
            time = db['data']['ts']
            data = db['data'][uc_vre_var]
            ...
            df_processed['variable'] = 'UC_VRE_Curtailment|' + df_processed['variable']
