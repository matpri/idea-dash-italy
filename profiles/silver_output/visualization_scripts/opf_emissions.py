...
def render_plot(type, df, scenarios, time_size='hourly'):
    ...
    if type == 'Total':
        ...
        opf_emissions = df[df['scenario'] == scenarios]
        opf_emissions['variable'] = opf_emissions.variable.apply(lambda x: x.split('|')[-1] if '|' in x else x)
        opf_emissions['month'] = opf_emissions['time'].dt.month
        opf_emissions['season'] = opf_emissions['month'].apply(
            lambda x: next((season for season, months in seasons.items() if x in months), 'Unknown'))

        seasoned = []
        for season in seasons.keys():
            print('Processing season:', season)
            opf_season = opf_emissions[opf_emissions['season'] == season].copy()
            try:
