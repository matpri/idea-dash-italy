...
def render_plot(type, df, scenarios, time_size='hourly'):
    ...
    name = plot_settings['UC_VRE_Curtailment']['name']
    unit = plot_settings['UC_VRE_Curtailment']['unit']

    title = plot_settings['UC_VRE_Curtailment'][type]['title']
    x_axis_label = plot_settings['UC_VRE_Curtailment'][type]['x_label']
    y_axis_label = plot_settings['UC_VRE_Curtailment'][type]['y_label']
    if type == 'Total':
...
def plot(df, window_id):
    ...
    widget_layout = html.Div([
        ...
        dmc.Button('Download Data', id={'type': 'silver-uc_vre_curtailment-download-button', 'index': window_id},
                   variant='light',
                   # center the button
                   style={'display': 'flex', 'justify-content': 'center', 'margin-top': '4px'}),
        dcc.Download(id={'type': 'silver-uc_vre_curtailment-download', 'index': window_id}),
    ])
