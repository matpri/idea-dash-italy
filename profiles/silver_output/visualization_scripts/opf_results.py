...
def render_plot(type, df, scenarios, time_size='hourly'):
    ...
    if type == 'Total':
        ...
        opf_results = df[df['scenario'] == scenarios]
        opf_results['variable'] = opf_results.variable.apply(lambda x: x.split('|')[-1] if '|' in x else x)
        opf_results['month'] = opf_results['time'].dt.month
        opf_results['season'] = opf_results['month'].apply(
            lambda x: next((season for season, months in seasons.items() if x in months), 'Unknown'))

        seasoned = []
        for season in seasons.keys():
            print('Processing season:', season)
            opf_season = opf_results[opf_results['season'] == season].copy()
            try:
                opf_season['day'] = opf_season['time'].dt.dayofyear
                season_daily_supply = opf_season.groupby(['day'], as_index=False)['value'].sum()
                ...
                min_df = opf_season[opf_season['day'] == season_daily_supply.iloc[min_day_idx]['day']][
                    ['value', 'variable']].groupby(by=['variable'], as_index=False).sum()
                min_df['date_type'] = 'Minimum Supply Day'
                max_df = opf_season[opf_season['day'] == season_daily_supply.iloc[max_day_idx]['day']][
                    ['value', 'variable']].groupby(by=['variable'], as_index=False).sum()
                max_df['date_type'] = 'Maximum Supply Day'
                avg = opf_season.groupby(['variable'], as_index=False)['value'].sum()
...
def plot(df, window_id):
    ...
    widget_layout = html.Div([
        dmc.Select(
            ...
            value='Total',
            id={
                'type': 'silver-opf_results-plot-select',
                'index': window_id
            },
        ),
        dmc.MultiSelect(
        ...
        dmc.Select(
            ...
            value=scenarios[0],
            id={
                'type': 'silver-opf_results-scenario-select',
                'index': window_id,
            },
            style={'display': 'none'}
        ),
        dmc.Select(
            ...
            value='hourly',
            id={
                'type': 'silver-opf_results-time_step-select',
                'index': window_id,
            },
            style={'display': 'block'}
        ),
