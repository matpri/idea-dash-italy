...
def render_plot(type, df, scenarios, time_size='hourly'):
    ...
    name = plot_settings['UC Results']['name']
    unit = plot_settings['UC Results']['unit']

    title = plot_settings['UC Results'][type]['title']
    x_axis_label = plot_settings['UC Results'][type]['x_label']
    y_axis_label = plot_settings['UC Results'][type]['y_label']
    if type == 'Total':
        ...
        uc_results = df[df['scenario'] == scenarios]
        uc_results['variable'] = uc_results.variable.apply(lambda x: x.split('|')[-1] if '|' in x else x)
        uc_results['month'] = uc_results['time'].dt.month
        uc_results['season'] = uc_results['month'].apply(
            lambda x: next((season for season, months in seasons.items() if x in months), 'Unknown'))

        seasoned = []
        for season in seasons.keys():
            print('Processing season:', season)
            uc_season = uc_results[uc_results['season'] == season].copy()
            try:
...
def plot(df, window_id):
    ...
    widget_layout = html.Div([
        ...
        dmc.Select(
            ...
            value=scenarios[0],
            id={
                'type': 'silver-uc_results-scenario-select',
                'index': window_id,
            },
            style={'display': 'none'}
        ),
        ...
        dmc.Button('Download Data', id={'type': 'silver-uc_results-download-button', 'index': window_id},
                   variant='light',
                   # center the button
                   style={'display': 'flex', 'justify-content': 'center', 'margin-top': '4px'}),
        dcc.Download(id={'type': 'silver-uc_results-download', 'index': window_id}),
    ])
