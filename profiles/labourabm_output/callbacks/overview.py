...
from profiles.labourabm_output.visualization_scripts.overview import render_plot
...
def link(app):
    @app.callback(
        ...
        Output({
            'type': 'labourabm-overview-download',
            'index': ALL
        }, 'data'),
        Input({
            'type': 'labourabm-overview-plot-select',
            'index': ALL
        }, 'value'),
        Input({
            'type': 'labourabm-overview-download-button',
            'index': ALL
        }, 'n_clicks'),
        State({
            'type': ids.FIGURE,
    ...
    def update_overview(_p_type, _download, _canvas, _data):
        ...
        if 'labourabm-overview-download-button' in trigger_id['type']:
            idx = 0
        ...
        for i, id in enumerate(ctx.inputs_list[0]):
            if ((id['id']['index'] == trigger_id['index']) and
                    (id['id']['type'] == 'labourabm-overview-plot-select')):
                idx = i
                break
