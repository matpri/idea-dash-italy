...
def check(df):
    ...
    print("Checking for total_employment in variable column")
    try:
        classes = df["variable"].apply(lambda x: x.split("|")[0])
        if (classes == 'total_employment').any():
            return True
        return False
...
def process(selected):
    ...
    for scenario, db in selected.items():
        df = db.copy()
        df = df[df['variable'].str.startswith('total_employment')]
        df['variable'] = df['variable'].str.replace('total_employment|', '')
