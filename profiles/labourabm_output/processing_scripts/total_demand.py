...
def check(df):
    ...
    print("Checking for total_demand in variable column")
    try:
        classes = df["variable"].apply(lambda x: x.split("|")[0])
        if (classes == 'total_demand').any():
            return True
        return False
...
def process(selected):
    ...
    for scenario, db in selected.items():
        df = db.copy()
        df = df[df['variable'].str.startswith('total_demand')]
        df['variable'] = df['variable'].str.replace('total_demand|', '')
