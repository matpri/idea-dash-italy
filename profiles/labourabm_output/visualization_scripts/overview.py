...
def render_plot(type, df, can=True):
    ...
    # name = plot_info['name']
    # unit = plot_info['unit']
    return plot_overview(df, type, 'Year', 'Value', type, '')
...
def plot(df, window_id):
    ...
    widget_layout = html.Div([
        ...
        dmc.Button('Download Data', id={'type': 'labourabm-overview-download-button', 'index': window_id},
                   variant='light',
                   # center the button
                   style={'display': 'flex', 'justify-content': 'center', 'margin-top': '4px'}),
        dcc.Download(id={'type': 'labourabm-overview-download', 'index': window_id}),
    ])
