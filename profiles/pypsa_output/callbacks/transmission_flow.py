...
def link(app):
    @app.callback(
        ...
        Output({
            'type': 'pypsa-transmissionflow-scenario-group-select',
            'index': ALL
        }, 'style'),
        Output({
        ...
        Output({
            'type': 'pypsa-transmissionflow-lines-select',
            'index': ALL
        }, 'style'),
        Output({
    ...
    def update_transmissionflow(_p_type, _scenarios, _scenario_group, _scenario, _years, _lines, _d_button, _canvas,
        ...
        if 'pypsa-transmissionflow-download-button' in trigger_id['type']:
            ...
            for i, id in enumerate(ctx.inputs_list[0]):
                if ((id['id']['index'] == trigger_id['index']) and
                    ...
                    idx = i
                    break
            _data[idx] = dcc.send_data_frame(data_handler.processed_data['NRCan-PyPsa']['Transmission Flow'].to_csv, "transmissionflow.csv")
        ...
        if _p_type[idx] == 'Map Plot':
            ...
            df = data_handler.processed_data['NRCan-PyPsa']['Transmission Flow']
