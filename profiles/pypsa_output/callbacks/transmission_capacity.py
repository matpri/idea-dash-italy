...
def link(app):
    @app.callback(
        ...
        Output({
            'type': 'pypsa-transmissioncapacity-download',
            'index': ALL
        }, 'data'),
        Input({
        ...
        Input({
            'type': 'pypsa-transmissioncapacity-download-button',
            'index': ALL
        }, 'n_clicks'),
        State({
            'type': ids.FIGURE,
    ...
    def update_transmissioncapacity(_p_type, _scenarios, _scenario_group, _scenario, _years, _lines, _d_button, _canvas,
        ...
        if 'pypsa-transmissioncapacity-download-button' in trigger_id['type']:
            idx = 0
            for i, id in enumerate(ctx.inputs_list[0]):
                if ((id['id']['index'] == trigger_id['index']) and
                        (id['id']['type'] == 'pypsa-transmissioncapacity-download-button')):
                    idx = i
                    break
            _data[idx] = dcc.send_data_frame(data_handler.processed_data['NRCan-PyPsa']['Transmission Capacity'].to_csv, "transmissioncapacity.csv")
        ...
        if _p_type[idx] == 'Map Plot':
            ...
            _canvas[idx] = render_plot('Map Plot',
                                       data_handler.processed_data['NRCan-PyPsa']['Transmission Capacity'],
                                       _scenario[idx],
                                       _years[idx],
                                        _lines[idx]
                                       )
        elif _p_type[idx] == 'Per Line Bar Plot':
            df = data_handler.processed_data['NRCan-PyPsa']['Transmission Capacity']
