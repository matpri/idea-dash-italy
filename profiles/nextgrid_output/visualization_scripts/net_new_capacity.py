...
def render_plot(type, df, aggregate, scenarios, region, year, scenario, pattern_active=True, text_active=False):
    from profiles.nextgrid_output.utils import plot_settings
...
def plot(df, window_id):
    ...
    by_year_widgets = dmc.Select(
        ...
        id={
            'type': 'nextgrid-netnew_capacity-region-select',
            'index': window_id
        },
        style={'display': 'block'}
    ...
    by_region_widgets = dmc.Select(
        ...
        value=years[0],
        id={
            'type': 'nextgrid-netnew_capacity-year-select',
            'index': window_id
        },
    ...
    pattern_toggle = dmc.Switch(
        ...
        checked=True,
        id={
            'type': 'nextgrid-netnew_capacity-pattern-switch',
            'index': window_id,
        },
        style={'display': 'block'}
    )
    ...
    text_toggle = dmc.Switch(
        ...
        checked=False,
        id={
            'type': 'nextgrid-netnew_capacity-text-switch',
            'index': window_id,
        },
        style={'display': 'block'}
    )
    ...
    widget_layout = html.Div([
        dmc.Select(
            ...
            value='By Year',
            id={
                'type': 'nextgrid-netnew_capacity-plot-select',
                'index': window_id
            },
        ),
        dmc.Switch('Aggregate',
                   checked=True,
                   id={
                       'type': 'nextgrid-netnew_capacity-aggregate-switch',
                       'index': window_id}),
        pattern_toggle,
        text_toggle,
        dmc.MultiSelect(
            ...
            value=[scenarios[0]],
            id={
                'type': 'nextgrid-netnew_capacity-scenario-multi-select',
                'index': window_id,
            },
            style={'display': 'block'}
        ),
        dmc.Select(
            ...
            value=scenarios[0],
            id={
                'type': 'nextgrid-netnew_capacity-scenario-select',
                'index': window_id,
            },
            style={'display': 'none'}
        ),
        by_year_widgets,
        by_region_widgets,
        dmc.Button('Download Data', id={'type': 'nextgrid-netnew_capacity-download-button', 'index': window_id},
                   variant='light',
                   # center the button
                     style={'display': 'flex', 'justify-content': 'center', 'margin-top': '4px'}),
        dcc.Download(id={'type': 'nextgrid-netnew_capacity-download', 'index': window_id}),
    ])
