...
def render_plot(df, p_type, r_type, by_rep, year, region, scenarios, scenario, variable, pattern_active=True,
    ...
    if r_type == 'By Year':
        return bar_over_years.plot(df_plt, scenarios, region, 'Petroleum Crude:' + variable, 'Year',
                                   variable,
                                   name, unit, pattern_active=pattern_active,
                                   text_active=text_active)
    elif r_type == 'Trend Over Years':
        return trend_over_years.plot(df_plt, scenario, region, 'Petroleum Crude:' + variable, 'Year',
                                   variable,
                                     name, unit)
    elif r_type == 'Pie Chart':
        return pie_chart.plot(df_plt, scenario, region, year, 'Petroleum Crude:' + variable, 'Year',
                                   variable)

    else:
        return bar_over_regions.plot(df_plt, scenarios, year, 'Petroleum Crude:' + 'Petroleum Crude:' + variable, 'Year',
                                   variable,
                                     name, unit, pattern_active=pattern_active,
                                     text_active=text_active)
...
def plot(df, window_id):
    ...
    widget_layout = html.Div([
        ...
        by_year_widgets,
        by_region_widgets,
        dmc.Button('Download Data', id={'type': 'cims-petroleum_crude-download-button', 'index': window_id},
                   variant='light',
                   # center the button
                   style={'display': 'flex', 'justify-content': 'center', 'margin-top': '4px'}),
        dcc.Download(id={'type': 'cims-petroleum_crude-download', 'index': window_id}),
    ])
