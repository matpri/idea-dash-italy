...
def plot(df, scenario, region, year, title, x_axis_label, y_axis_label, season=None):
    fig = go.Figure()
    fig.update_layout(
        title_text=title,
    ...
    try:
        df_scen = subset(df, region, year, scenario, season)
...
def subset(df, region, year, scenario, season=None):
