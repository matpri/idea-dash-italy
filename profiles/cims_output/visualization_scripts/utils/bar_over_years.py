...
def plot(df, scenarios, region, title, x_axis_label, y_axis_label, tooltip_name, unit, season=None, pattern_active=True, text_active=False):
    fig = go.Figure()
    ...
    try:
        df_scen = subset(df, region, scenarios, season)
        scenarios.sort()
...
def subset(df, region, scenarios, season=None):
