...
from profiles.cims_output.visualization_scripts.light_industrial import render_plot
...
def link(app):
    @app.callback(
        ...
        Output({
            'type': 'cims-light_industrial-download',
            'index': MATCH
        }, 'data'),
        Input({
        ...
        Input({
            'type': 'cims-light_industrial-download-button',
            'index': MATCH
        }, 'n_clicks'),
    ...
    def update_plot(plot_type, plot, region, year, variable, scenarios, scenario, rep_switch, pattern_switch,
        ...
        print('Light Industrial')
        from main import data_handler
        ctx = dash.callback_context
        trigger_id = eval(ctx.triggered[0]['prop_id'].split('.')[0])
        _data = data_handler.processed_data['CIMS Output']['Light Industrial']

        if trigger_id['type'] == 'cims-light_industrial-download-button':
            return (dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update,
                    dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update,
                    dcc.send_data_frame(_data.to_csv, "light_industrial.csv"))
        ...
        if trigger_id['type'] == 'cims-light_industrial-plot-select':
