...
from profiles.cims_output.visualization_scripts.metal_smelting import render_plot
...
def link(app):
    @app.callback(
        ...
        Output({
            'type': 'cims-metal_smelting-download',
            'index': MATCH
        }, 'data'),
        Input({
        ...
        Input({
            'type': 'cims-metal_smelting-download-button',
            'index': MATCH
        }, 'n_clicks'),
    ...
    def update_plot(plot_type, plot, region, year, variable, scenarios, scenario, rep_switch, pattern_switch,
        ...
        print('Metal Smelting')
        from main import data_handler
        ctx = dash.callback_context
        trigger_id = eval(ctx.triggered[0]['prop_id'].split('.')[0])
        _data = data_handler.processed_data['CIMS Output']['Metal Smelting']

        if trigger_id['type'] == 'cims-metal_smelting-download-button':
            return (dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update,
                    dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update,
                    dcc.send_data_frame(_data.to_csv, "metal_smelting.csv"))
        ...
        if trigger_id['type'] == 'cims-metal_smelting-plot-select':
