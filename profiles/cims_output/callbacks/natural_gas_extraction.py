...
from profiles.cims_output.visualization_scripts.natural_gas_extraction import render_plot
...
def link(app):
    @app.callback(
        ...
        Output({
            'type': 'cims-natural_gas_extraction-download',
            'index': MATCH
        }, 'data'),
        Input({
        ...
        Input({
            'type': 'cims-natural_gas_extraction-download-button',
            'index': MATCH
        }, 'n_clicks'),
    ...
    def update_plot(plot_type, plot, region, year, variable, scenarios, scenario, rep_switch, pattern_switch,
        ...
        _data = data_handler.processed_data['CIMS Output']['Natural Gas Extraction']

        if trigger_id['type'] == 'cims-natural_gas_extraction-download-button':
            return (dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update,
                    dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update,
                    dcc.send_data_frame(_data.to_csv, "natural_gas_extraction.csv"))
        ...
        if trigger_id['type'] == 'cims-natural_gas_extraction-plot-select':
