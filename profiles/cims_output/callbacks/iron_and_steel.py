...
from profiles.cims_output.visualization_scripts.iron_and_steel import render_plot
...
def link(app):
    @app.callback(
        ...
        Output({
            'type': 'cims-iron_and_steel-download',
            'index': MATCH
        }, 'data'),
        Input({
        ...
        Input({
            'type': 'cims-iron_and_steel-download-button',
            'index': MATCH
        }, 'n_clicks'),
    ...
    def update_plot(plot_type, plot, region, year, variable, scenarios, scenario, rep_switch, pattern_switch,
        ...
        print('Iron and Steel')
        from main import data_handler
        ctx = dash.callback_context
        trigger_id = eval(ctx.triggered[0]['prop_id'].split('.')[0])
        _data = data_handler.processed_data['CIMS Output']['Iron and Steel']

        if trigger_id['type'] == 'cims-iron_and_steel-download-button':
            return (dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update,
                    dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update,
                    dcc.send_data_frame(_data.to_csv, "iron_and_steel.csv"))
        ...
        if trigger_id['type'] == 'cims-iron_and_steel-plot-select':
