...
from profiles.temoa_output.callbacks import (emissions as emissions_callbacks,
...
from profiles.temoa_output.processing_scripts import (
...
from profiles.temoa_output.visualization_scripts import (
...
class PypsaOutput(BaseProfile):
    ...
    def __init__(self):
        super().__init__()
        self.technologies = yaml.load(open('./profiles/temoa_output/technologies.yaml', 'r'), Loader=yaml.FullLoader)
        self.plots = yaml.load(open('./profiles/temoa_output/plots.yaml', 'r'), Loader=yaml.FullLoader)
        self.update_utils()
    ...
    def render_settings(self):
        layout = html.Div(
            [
                ...
                html.Div(id='temoa-settings-upload-yaml-output'),
                dmc.Tabs([
                    dmc.TabsList([
                        dmc.Tab('Technology Settings', id='temoa-technologies', value='tech'),
                        dmc.Tab('Plot Settings', id='temoa-plot-settings', value='plot'),
                    ]
                    ),
                    dmc.TabsPanel(id='temoa-technologies-settings', value='tech',
                                  children=self.render_technology_settings()),
                    dmc.TabsPanel(id='temoa-plot-settings-panel', value='plot',
    ...
    def render_technology_settings(self):
        ...
        layout = html.Div([
            ...
            html.Div(utils.tech_edit(techs[0]),
                     id='temoa-technology-settings-output'),
        ])
