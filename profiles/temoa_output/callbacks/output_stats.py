...
from profiles.temoa_output.visualization_scripts.output_stats import render_plot
...
def link(app):
    @app.callback(
        ...
        Output({
            'type': 'temoa_output-output_stats-download',
            'index': ALL
        }, 'data'),
        Input({
        ...
        Input({
            'type': 'temoa_output-output_stats-download-button',
            'index': ALL
        }, 'n_clicks'),
        Input({
    ...
    def update_output_stats(_years, _download, _scenarios, _canvas, _data):
        ...
        if 'temoa_output-output_stats-download-button' in trigger_id['type']:
            idx = 0
        ...
        for i, id in enumerate(ctx.inputs_list[0]):
            if ((id['id']['index'] == trigger_id['index']) and
                    (id['id']['type'] == 'temoa_output-output_stats-plot-select')):
                idx = i
                break
