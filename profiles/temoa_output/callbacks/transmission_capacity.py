    ...
    def update_transmissioncapacity(_p_type, _scenarios, _scenario_group, _scenario, _years, _lines, _d_button, _canvas,
        ...
        if 'temoa-transmissioncapacity-download-button' in trigger_id['type']:
            ...
            for i, id in enumerate(ctx.inputs_list[0]):
                if ((id['id']['index'] == trigger_id['index']) and
                    ...
                    idx = i
                    break
            _data[idx] = dcc.send_data_frame(data_handler.processed_data['Sutubra']['Transmission Capacity'].to_csv, "transmissioncapacity.csv")
        ...
        if _p_type[idx] == 'Map Plot':
            ...
            _canvas[idx] = render_plot('Map Plot',
                                       data_handler.processed_data['Sutubra']['Transmission Capacity'],
                                       _scenario[idx],
                                       _years[idx],
                                        _lines[idx]
                                       )
        elif _p_type[idx] == 'Per Line Bar Plot':
            df = data_handler.processed_data['Sutubra']['Transmission Capacity']
