...
def check(df):
    ...
    try:
        if (df.model == 'Sutubra-TEMOA').any():
            if df.variable.str.startswith("Capital|Variable costs|").any():
                return df[df.variable.str.startswith("Capital|Variable costs|")]['value'].sum() != 0
        return False
...
def process(data):
    ...
    for scenario_name, db in data.items():
        df = db.copy()
        df = df[df.variable.str.startswith("Capital|Variable costs|")]
