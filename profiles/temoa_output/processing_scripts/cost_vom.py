...
def check(df):
    ...
    try:
        if (df.model == 'Sutubra').any():
            if df.variable.str.startswith("Operational|VOM costs (nom_undisc)|").any():
                return df[df.variable.str.startswith("Operational|VOM costs (nom_undisc)|")]['value'].sum() != 0
        return False
...
def calculate_fom(fom_df):
    """
    Calculates the fixed operating and maintenance (VOM) cost data from the DataFrame.
...
def process(data):
    ...
    for scenario_name, db in data.items():
        df = db.copy()
        df = df[df.variable.str.startswith("Operational|VOM costs (nom_undisc)|")]
