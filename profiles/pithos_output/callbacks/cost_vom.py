...
from profiles.pithos_output.visualization_scripts.cost_vom import render_plot
...
def link(app):
    @app.callback(
        ...
        Input({
            'type': 'pithos-vom_cost-download-button',
            'index': ALL
        }, 'n_clicks'),
        State({
            'type': 'pithos-vom_cost-region-select',
    ...
    def update_vom_cost(_p_type, _aggregates, _scenarios, _scenario, _regions, _years, _pattern, _text,
        ...
        if 'pithos-vom_cost-download-button' in trigger_id['type']:
            idx = 0
            for i, id in enumerate(ctx.inputs_list[0]):
                if ((id['id']['index'] == trigger_id['index']) and
                        (id['id']['type'] == 'pithos-vom_cost-download-button')):
                    idx = i
                    break
            _data[idx] = dcc.send_data_frame(data_handler.processed_data['ESMIA-PITHOS Output']['VOM Cost'].to_csv, "vom_cost.csv")
            return _canvas, _r_style, _y_style, _data, _s_style, _m_style

        idx = 0
        for i, id in enumerate(ctx.inputs_list[0]):
            if ((id['id']['index'] == trigger_id['index']) and
                    (id['id']['type'] == 'pithos-vom_cost-plot-select')):
                idx = i
                break
        ...
        if _p_type[idx] == 'By Year':
            ...
            if _aggregates[idx] is not None:
                _canvas[idx] = render_plot('By Year', data_handler.processed_data['ESMIA-PITHOS Output']['VOM Cost'],
                                           _aggregates[idx],
                                           _scenarios[idx],
                                           _regions[idx],
            ...
            if _aggregates[idx] is not None:
                _canvas[idx] = render_plot('Trend Over Years', data_handler.processed_data['ESMIA-PITHOS Output']['VOM Cost'],
                                           _aggregates[idx],
                                           _scenarios[idx],
                                           _regions[idx],
                                           _years[idx], scenario=_scenario[idx])
            ...
            _text_style[idx] = {'display': 'none'} 
            if _aggregates[idx] is not None:
                _canvas[idx] = render_plot('Pie Chart', data_handler.processed_data['ESMIA-PITHOS Output']['VOM Cost'],
                                           _aggregates[idx],
                                           _scenarios[idx],
                                           _regions[idx],
                                           _years[idx], scenario=_scenario[idx])
            ...
            _text_style[idx] = {'display': 'block'}
            if _aggregates[idx] is not None:
                _canvas[idx] = render_plot('By Region', data_handler.processed_data['ESMIA-PITHOS Output']['VOM Cost'],
                                           _aggregates[idx],
                                           _scenarios[idx],
                                           _regions[idx],
                                           _years[idx], scenario=_scenario[idx],
