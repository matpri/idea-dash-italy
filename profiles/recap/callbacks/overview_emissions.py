...
def link(app):
    """
    Link callbacks for the Emissions-only overview visualization.
    No tab switching needed - only emissions functionality.
    """
    # Link the ghg callbacks for emissions functionality
    ghg.link(app)
    
    # No additional callbacks needed since ghg.link(app) handles everything
    # The ghg callbacks already manage all the emissions visualization logic
    @app.callback(
        Output({
            'type': 'recap-overview-stocks',
            'index': ALL
