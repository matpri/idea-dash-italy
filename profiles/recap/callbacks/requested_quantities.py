...
from profiles.recap.visualization_scripts.requested_quantities import render_plot
...
def link(app):
    @app.callback(
        ...
        Input({
            'type': 'recap-requested_quantities-download-button',
            'index': MATCH
        }, 'n_clicks'),
        Input({
            'type': 'recap-requested_quantities-sector-select',
    ...
    def update_requested_quantities(_update, _representation, _p_type, _scenarios, _scenario, _regions, _years,
        ...
        _data = data_handler.processed_data['Summary']['Electricity Demand']
        _data = _data[_data['tab'] == 'Energy Demand']

        layers = [dash.no_update] * len(_service_style)
        service_value = _service.copy()
        if 'recap-requested_quantities-download-button' in trigger_id['type']:
        ...
        if 'recap-requested_quantities-sector-select' in trigger_id['type']:
        ...
        if 'recap-requested_quantities-service-select' in trigger_id['type']:
            _data = _data[_data['sector'] == _sector]
