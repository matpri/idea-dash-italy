...
from profiles.recap.visualization_scripts.cost_total import render_plot
...
def link(app):
    @app.callback(
        ...
        Output({
            'type': 'recap-total_cost-year-select',
            'index': ALL
        }, 'style'),
        Output({
        ...
        Output({
            'type': 'recap-total_cost-scenario-select',
            'index': ALL
        }, 'style'),
        Output({
            'type': 'recap-total_cost-scenario-multi-select',
            'index': ALL
        }, 'style'),
        Output({
            'type': 'recap-total_cost-scenario-group-select',
            'index': ALL
        }, 'style'),
        Output(
            {
                'type': 'recap-total_cost-pattern-switch',
                'index': ALL
            },
            'style'
        ),
        Output(
            {
                'type': 'recap-total_cost-text-switch',
                'index': ALL
            },
            'style'
        ),
        Input({
            'type': 'recap-total_cost-plot-select',
            'index': ALL
        }, 'value'),
        Input({
            'type': 'recap-total_cost-aggregate-switch',
            'index': ALL
        }, 'checked'),
        Input({
            'type': 'recap-total_cost-scenario-multi-select',
        ...
        Input({
            'type': 'recap-total_cost-download-button',
            'index': ALL
        }, 'n_clicks'),
        State({
            'type': 'recap-total_cost-region-select',
    ...
    def update_total_cost(_p_type, _aggregates, _scenarios, _scenario_group, _scenario, _regions, _years, _pattern, _text,
        ...
        if 'recap-total_cost-download-button' in trigger_id['type']:
            idx = 0
        ...
        for i, id in enumerate(ctx.inputs_list[0]):
            if ((id['id']['index'] == trigger_id['index']) and
                    (id['id']['type'] == 'recap-total_cost-plot-select')):
                idx = i
                break
