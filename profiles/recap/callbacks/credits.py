...
def link(app):
    @app.callback(
        Output({'type': ids.FIGURE, 'index': MATCH, 'profile': 'recap', 'viz': 'Credits'}, 'figure'),
        ...
        # States
        State({'type': ids.FIGURE, 'index': MATCH, 'profile': 'recap', 'viz': 'Credits'}, 'figure'),
    ...
    def update_credits_plot(
        ...
        if 'recap-credits-download-button' in trigger_id['type']:
            download_data = dcc.send_data_frame(
                data_handler.processed_data['recap']['Credits'].to_csv,
                "credits.csv"
            )
            return (
        ...
        # Get the data
        credits_data = data_handler.processed_data['recap']['Credits']
