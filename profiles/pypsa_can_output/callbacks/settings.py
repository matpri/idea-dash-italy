...
def link(app):
    @app.callback(
    Output('pypsa_can-settings-upload-yaml-output', 'children'),
    Input('pypsa_can-settings-upload-yaml', 'contents'),
    State('pypsa_can-settings-upload-yaml', 'filename'),
    )
    ...
    @app.callback(
        Output('pypsa_can-technology-settings-output', 'children'),
        Input('pypsa_can-technology-select', 'value')
    )
    ...
    @app.callback(
        Output('pypsa_can-plot-settings-output', 'children'),
        Input('pypsa_can-plot-select', 'value')
    )
    ...
    @app.callback(
        Output({'type': 'pypsa_can-tech-update', 'index': ALL}, 'disabled'),
        Input({'type': 'pypsa_can-tech-update', 'index': ALL}, 'n_clicks'),
        Input({'type': 'pypsa_can-tech-name', 'index': ALL}, 'value'),
        Input({'type': 'pypsa_can-tech-group', 'index': ALL}, 'value'),
        Input({'type': 'pypsa_can-tech-color', 'index': ALL}, 'value'),
        Input({'type': 'pypsa_can-tech-group-color', 'index': ALL}, 'value'),
        State({'type': 'pypsa_can-tech-update', 'index': ALL}, 'disabled'),
    ...
    def tech_update(n_clicks, names, groups, colors, group_colors, disabled):
        ...
        if trigger_id['type'] == 'pypsa_can-tech-update':
            disabled[idx] = True
    ...
    @app.callback(
        Output({'type': 'pypsa_can-plot-update', 'index': ALL, 'subtype':ALL}, 'disabled'),
        Input({'type': 'pypsa_can-plot-update', 'index': ALL, 'subtype':ALL}, 'n_clicks'),
        Input({'type': 'pypsa_can-plot-title', 'index': ALL, 'subtype':ALL}, 'value'),
        Input({'type': 'pypsa_can-plot-x-axis', 'index': ALL, 'subtype':ALL}, 'value'),
        Input({'type': 'pypsa_can-plot-y-axis', 'index': ALL, 'subtype':ALL}, 'value'),
        State({'type': 'pypsa_can-plot-update', 'index': ALL, 'subtype':ALL}, 'disabled'),
        prevent_initial_call=True
    )
    def plot_update(n_clicks, titles, x_axis, y_axis, disabled):
        ...
        for i, out in enumerate(ctx.outputs_list):
            if out['id']['index'] == trigger_id['index']:
                if trigger_id['type'] == 'pypsa_can-plot-update':
                    disabled[idx] = True
