...
from profiles.pypsa_can_output.visualization_scripts.transmission_flow import render_plot
...
def link(app):
    @app.callback(
        ...
        Output({
            'type': 'pypsa_can-transmissionflow-scenario-multi-select',
            'index': ALL
        }, 'style'),
        Output({
            'type': 'pypsa_can-transmissionflow-scenario-group-select',
            'index': ALL
        }, 'style'),
        Output({
            'type': 'pypsa_can-transmissionflow-year-select',
            'index': ALL
        }, 'style'),
        Output({
            'type': 'pypsa_can-transmissionflow-lines-select',
            'index': ALL
        }, 'style'),
        Output({
            'type': 'pypsa_can-transmissionflow-download',
            'index': ALL
        }, 'data'),
        Input({
            'type': 'pypsa_can-transmissionflow-plot-select',
            'index': ALL
        }, 'value'),
        Input({
            'type': 'pypsa_can-transmissionflow-scenario-multi-select',
        ...
        Input({
            'type': 'pypsa_can-transmissionflow-download-button',
            'index': ALL
        }, 'n_clicks'),
        State({
            'type': ids.FIGURE,
    ...
    def update_transmissionflow(_p_type, _scenarios, _scenario_group, _scenario, _years, _lines, _d_button, _canvas,
        ...
        if 'pypsa_can-transmissionflow-download-button' in trigger_id['type']:
            idx = 0
            for i, id in enumerate(ctx.inputs_list[0]):
                if ((id['id']['index'] == trigger_id['index']) and
                        (id['id']['type'] == 'pypsa_can-transmissionflow-download-button')):
                    idx = i
                    break
            _data[idx] = dcc.send_data_frame(data_handler.processed_data['PyPSA_CAN']['Transmission Flow'].to_csv, "transmissionflow.csv")
        ...
        if _p_type[idx] == 'Map Plot':
            ...
            df = data_handler.processed_data['PyPSA_CAN']['Transmission Flow']
