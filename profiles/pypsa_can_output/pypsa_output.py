...
from profiles.pypsa_can_output.callbacks import (emissions as emissions_callbacks,
...
from profiles.pypsa_can_output.processing_scripts import (
...
from profiles.pypsa_can_output.visualization_scripts import (
...
class PypsaOutput(BaseProfile):
    display_name = 'PyPSA_CAN'
    name = 'PyPSA_CAN'
    db_name = 'pypsa_can'
    color = 'yellow 8'
    description = (
        'The Canadian Opportunities for Planning and Production of Electricity Resources (PyPSA_CAN) framework is an electricity system planning model. \n'
    ...
    def __init__(self):
        super().__init__()
        self.technologies = yaml.load(open('./profiles/pypsa_can_output/technologies.yaml', 'r'), Loader=yaml.FullLoader)
        self.plots = yaml.load(open('./profiles/pypsa_can_output/plots.yaml', 'r'), Loader=yaml.FullLoader)
        self.update_utils()
    ...
    def render_settings(self):
        layout = html.Div(
            [
                # upload for yaml
                dcc.Upload(
                    id='pypsa_can-settings-upload-yaml',
                    children=html.Div([
                        'Drag and Drop or ',
                ...
                html.Div(id='pypsa_can-settings-upload-yaml-output'),
                dmc.Tabs([
                    dmc.TabsList([
                        dmc.Tab('Technology Settings', id='pypsa_can-technologies', value='tech'),
                        dmc.Tab('Plot Settings', id='pypsa_can-plot-settings', value='plot'),
                    ]
                    ),
                    dmc.TabsPanel(id='pypsa_can-technologies-settings', value='tech',
                                  children=self.render_technology_settings()),
                    dmc.TabsPanel(id='pypsa_can-plot-settings-panel', value='plot',
    ...
    def render_technology_settings(self):
        ...
        layout = html.Div([
            html.Div(
                dmc.Select(
                    id='pypsa_can-technology-select',
            ...
            html.Div(utils.tech_edit(techs[0]),
                     id='pypsa_can-technology-settings-output'),
        ])
    ...
    def render_plot_settings(self):
        ...
        layout = html.Div([
            ...
            html.Div(utils.plot_edit(plots[0]),
                     id='pypsa_can-plot-settings-output'),
        ])
