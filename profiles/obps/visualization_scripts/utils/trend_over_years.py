...
def plot(df, scenario, region, aggregate, title, x_axis_label, y_axis_label, tooltip_name, ):
    fig = go.Figure()
    fig.update_layout(
        title_text=title,
    ...
    try:
        df_scen = subset(df, region, scenario, aggregate)
...
def subset(df, region, scenario, aggregate):
    df_scen = df.copy(deep=True)

    df_scen['variable'] = df_scen['variable'] + ' (' + df_scen['unit'] + ')'
