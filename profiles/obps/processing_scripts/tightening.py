...
def check(df):
    ...
    try:
        if (df.model == 'OBPS').any():
            if df.variable.str.contains("Tightening").any():
                return True
        return False
    except Exception as e:
        print("OBPS tightening  check", e)
        return False
...
def process(dbs: dict):
    ...
    for scenario_name, db in dbs.items():
        df = db.copy()
        df = df[df.variable.str.contains('Tightening')]
        df['variable'] = df['variable'].str.replace('Input|Policy|OBPS|Tightening|', '')
