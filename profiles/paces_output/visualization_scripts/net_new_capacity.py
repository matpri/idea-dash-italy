...
from profiles.paces_output.visualization_scripts.utils import bar_over_years, bar_over_regions, trend_over_years, pie_chart


def render_plot(type, df, aggregate, scenarios, region, year, scenario, pattern_active=True, text_active=False):
    from profiles.paces_output.utils import plot_settings
    #print('rendering plot', type)
    name = plot_settings['Net New Capacity']['name']
    unit = plot_settings['Net New Capacity']['unit']
    if type == 'By Year':
        plot_info = plot_settings['Net New Capacity']['By Year']
...
def plot(df, window_id):
    ...
    widget_layout = html.Div([
        ...
        by_year_widgets,
        by_region_widgets,
        dmc.Button('Download Data', id={'type': 'paces-netnew_capacity-download-button', 'index': window_id},
                   variant='light',
                   # center the button
                     style={'display': 'flex', 'justify-content': 'center', 'margin-top': '4px'}),
        dcc.Download(id={'type': 'paces-netnew_capacity-download', 'index': window_id}),
    ])
