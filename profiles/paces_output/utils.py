        ...
        html.Div(children=[
            html.Div([
                dmc.TextInput(
                    label='Unaggregated',
                    value=names[tech],
                    id={'type': 'paces-tech-name', 'index': tech}
                ),
                daq.ColorPicker(
                    value={'hex': colors[tech]},
                    id={'type': 'paces-tech-color', 'index': tech}
                )],
                style=glass_style
            ),
            html.Div([
                dmc.TextInput(
                    label='Aggregated',
                    value=groups[tech],
                    id={'type': 'paces-tech-group', 'index': tech}
                ),
                daq.ColorPicker(
                    value={'hex': group_colors[groups[tech]]},
                    id={'type': 'paces-tech-group-color', 'index': tech}
                )],
                style=glass_style
            )
        ], style={'display': 'flex', 'justifyContent': 'space-around'}),
        html.Div(
            dmc.Button('Update', id={'type': 'paces-tech-update', 'index': tech}, disabled=True,
...
def plot_edit(plot):
    ...
    for i, plot_type in enumerate(types):
        views.append(html.Div([
            ...
            dmc.TextInput(
                ...
                id={'type': 'paces-plot-title', 'index': plot, 'subtype': plot_type}
            ),
            dmc.TextInput(
                label='Y Axis Label',
                value=plots[plot_type]['y_label'],
                id={'type': 'paces-plot-y-axis', 'index': plot, 'subtype': plot_type}
            ),
            dmc.TextInput(
                label='X Axis Label',
                value=plots[plot_type]['x_label'],
                id={'type': 'paces-plot-x-axis', 'index': plot, 'subtype': plot_type}
            ),
            dmc.Divider(),
        ],
            style=glass_style
    ...
    layout = html.Div([
        *views,
        html.Div(
            dmc.Button('Update', id={'type': 'paces-plot-update', 'index': plot, 'subtype': '-'.join(types)},
                       disabled=True,
                       style={'display': 'flex', 'justifyContent': 'center', 'width': '80%'}),
            style={'display': 'flex', 'justifyContent': 'center'}
        )
