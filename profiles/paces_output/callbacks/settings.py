...
def link(app):
    @app.callback(
    Output('paces-settings-upload-yaml-output', 'children'),
    Input('paces-settings-upload-yaml', 'contents'),
    State('paces-settings-upload-yaml', 'filename'),
    )
    def update_settings(content, filename):
        ...
        data_handler.profiles['PaCES'].settings = settings
        return html.Div([
    ...
    @app.callback(
        Output('paces-technology-settings-output', 'children'),
        Input('paces-technology-select', 'value')
    )
    ...
    @app.callback(
        Output('paces-plot-settings-output', 'children'),
        Input('paces-plot-select', 'value')
    )
    ...
    @app.callback(
        Output({'type': 'paces-tech-update', 'index': ALL}, 'disabled'),
        Input({'type': 'paces-tech-update', 'index': ALL}, 'n_clicks'),
        Input({'type': 'paces-tech-name', 'index': ALL}, 'value'),
        Input({'type': 'paces-tech-group', 'index': ALL}, 'value'),
        Input({'type': 'paces-tech-color', 'index': ALL}, 'value'),
        Input({'type': 'paces-tech-group-color', 'index': ALL}, 'value'),
        State({'type': 'paces-tech-update', 'index': ALL}, 'disabled'),
    ...
    def tech_update(n_clicks, names, groups, colors, group_colors, disabled):
        ...
        if trigger_id['type'] == 'paces-tech-update':
            disabled[idx] = True
    ...
    @app.callback(
        Output({'type': 'paces-plot-update', 'index': ALL, 'subtype':ALL}, 'disabled'),
        Input({'type': 'paces-plot-update', 'index': ALL, 'subtype':ALL}, 'n_clicks'),
        Input({'type': 'paces-plot-title', 'index': ALL, 'subtype':ALL}, 'value'),
        Input({'type': 'paces-plot-x-axis', 'index': ALL, 'subtype':ALL}, 'value'),
        Input({'type': 'paces-plot-y-axis', 'index': ALL, 'subtype':ALL}, 'value'),
        State({'type': 'paces-plot-update', 'index': ALL, 'subtype':ALL}, 'disabled'),
        prevent_initial_call=True
    )
    def plot_update(n_clicks, titles, x_axis, y_axis, disabled):
        ...
        for i, out in enumerate(ctx.outputs_list):
            if out['id']['index'] == trigger_id['index']:
                if trigger_id['type'] == 'paces-plot-update':
                    disabled[idx] = True
