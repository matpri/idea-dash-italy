...
from profiles.paces_output.visualization_scripts.qualifying_capacity import render_plot
...
def link(app):
    @app.callback(
        ...
        Output({
            'type': 'paces-qualifying-capacity-region-select',
            'index': ALL
        }, 'style'),
        Output({
            'type': 'paces-qualifying-capacity-year-select',
            'index': ALL
        }, 'style'),
        Output({
            'type': 'paces-qualifying-capacity-download',
            'index': ALL
        }, 'data'),
        Output({
            'type': 'paces-qualifying-capacity-scenario-select',
            'index': ALL
        }, 'style'),
        Output({
            'type': 'paces-qualifying-capacity-scenario-multi-select',
            'index': ALL
        }, 'style'),
        Output(
            {
                'type': 'paces-qualifying-capacity-pattern-switch',
                'index': ALL
            },
            'style'
        ),
        Output(
            {
                'type': 'paces-qualifying-capacity-text-switch',
                'index': ALL
            },
            'style'
        ),
        Input({
            'type': 'paces-qualifying-capacity-plot-select',
            'index': ALL
        }, 'value'),
        Input({
            'type': 'paces-qualifying-capacity-aggregate-switch',
            'index': ALL
        }, 'checked'),
        Input({
            'type': 'paces-qualifying-capacity-scenario-multi-select',
        ...
        Input({
            'type': 'paces-qualifying-capacity-download-button',
            'index': ALL
        }, 'n_clicks'),
        Input({
            'type': 'paces-qualifying-capacity-season-select',
            'index': ALL
        }, 'value'),
        State({
            'type': 'paces-qualifying-capacity-region-select',
    ...
    def update_net_new_capacity(_p_type, _aggregates, _scenarios, _scenario, _regions, _years, _pattern, _text,_download, _seasons,
        ...
        if 'paces-qualifying-capacity-download-button' in trigger_id['type']:
            idx = 0
            for i, id in enumerate(ctx.inputs_list[0]):
                if ((id['id']['index'] == trigger_id['index']) and
                        (id['id']['type'] == 'paces-qualifying-capacity-download-button')):
                    idx = i
                    break
            _data[idx] = dcc.send_data_frame(data_handler.processed_data['PaCES']['Capacity'].to_csv,
        ...
        for i, id in enumerate(ctx.inputs_list[0]):
            if ((id['id']['index'] == trigger_id['index']) and
                    (id['id']['type'] == 'paces-qualifying-capacity-plot-select')):
                idx = i
                break
        ...
        if _p_type[idx] == 'By Year':
            ...
            if _aggregates[idx] is not None:
                _canvas[idx] = render_plot('By Year',
                                           data_handler.processed_data['PaCES']['Qualifying Capacity'],
                                           _aggregates[idx],
                                           _scenarios[idx],
                                           _regions[idx],
                                           _years[idx],
