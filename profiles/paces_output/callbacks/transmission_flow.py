...
from profiles.paces_output.visualization_scripts.transmission_flow import render_plot
...
def link(app):
    @app.callback(
        ...
        Output({
            'type': 'paces-transmissionflow-scenario-select',
            'index': ALL
        }, 'style'),
        Output({
            'type': 'paces-transmissionflow-scenario-multi-select',
            'index': ALL
        }, 'style'),
        Output({
            'type': 'paces-transmissionflow-scenario-group-select',
            'index': ALL
        }, 'style'),
        Output({
            'type': 'paces-transmissionflow-year-select',
            'index': ALL
        }, 'style'),
        Output({
            'type': 'paces-transmissionflow-lines-select',
            'index': ALL
        }, 'style'),
        Output({
            'type': 'paces-transmissionflow-download',
            'index': ALL
        }, 'data'),
        Input({
            'type': 'paces-transmissionflow-plot-select',
            'index': ALL
        }, 'value'),
        Input({
            'type': 'paces-transmissionflow-scenario-multi-select',
        ...
        Input({
            'type': 'paces-transmissionflow-download-button',
            'index': ALL
        }, 'n_clicks'),
        State({
            'type': ids.FIGURE,
    ...
    def update_transmissionflow(_p_type, _scenarios, _scenario_group, _scenario, _years, _lines, _d_button, _canvas,
        ...
        if 'paces-transmissionflow-download-button' in trigger_id['type']:
            idx = 0
            for i, id in enumerate(ctx.inputs_list[0]):
                if ((id['id']['index'] == trigger_id['index']) and
                        (id['id']['type'] == 'paces-transmissionflow-download-button')):
                    idx = i
                    break
            _data[idx] = dcc.send_data_frame(data_handler.processed_data['PaCES']['Transmission Flow'].to_csv, "transmissionflow.csv")
        ...
        if _p_type[idx] == 'Map Plot':
            ...
            _canvas[idx] = render_plot('Map Plot',
                                       data_handler.processed_data['PaCES']['Transmission Flow'],
                                       _scenario[idx],
                                       _years[idx],
                                        _lines[idx]
                                       )
        elif _p_type[idx] == 'Per Line Bar Plot':
            df = data_handler.processed_data['PaCES']['Transmission Flow']
