...
from profiles.paces_output.visualization_scripts.dispatch import render_plot, date_mapper
...
def link(app):
    @app.callback(
        ...
        Output({
            'type': 'paces-dispatch-region-select',
            'index': ALL
        }, 'data'),
        Output({
            'type': 'paces-dispatch-region-select',
        ...
        Output({
            'type': 'paces-dispatch-year-select',
            'index': ALL
        }, 'data'),
        Output({
            'type': 'paces-dispatch-year-select',
        ...
        Output({
            'type': 'paces-dispatch-day-select',
            'index': ALL
        }, 'data'),
        Output({
            'type': 'paces-dispatch-day-select',
        ...
        Output({
            'type': 'paces-dispatch-download',
            'index': ALL
        }, 'data'),
        Input({
            'type': 'paces-dispatch-plot-select',
            'index': ALL
        }, 'value'),
        Input({
            'type': 'paces-dispatch-aggregate-switch',
            'index': ALL
        }, 'checked'),
        Input({
            'type': 'paces-dispatch-scenario-multi-select',
            'index': ALL
        }, 'value'),
        Input({
            'type': 'paces-dispatch-region-select',
        ...
        Input({
            'type': 'paces-dispatch-download-button',
            'index': ALL
        }, 'n_clicks'),
        State({
            'type': 'paces-dispatch-region-select',
    ...
    def dispatch_callback(plot_select, aggregate_switch, scenario_multi_select, region_select, year_select, day_select,
        ...
        if 'paces-dispatch-download-button' in trigger_id['type']:
            idx = 0
            for i, id in enumerate(ctx.inputs_list[0]):
                if ((id['id']['index'] == trigger_id['index']) and
                        (id['id']['type'] == 'paces-dispatch-download-button')):
                    idx = i
                    break
            download[idx] = dcc.send_data_frame(data_handler.processed_data['PaCES']['Dispatch'].to_csv,
                                                "dispatch.csv")
            return figure, region_data, region_value, year_data, year_value, day_data, day_value, download

        if 'paces-dispatch-scenario-multi-select' in trigger_id['type']:
            df = data_handler.processed_data['PaCES']['Dispatch']
            idx = 0
            for i, id in enumerate(ctx.inputs_list[0]):
                if ((id['id']['index'] == trigger_id['index']) and
                        (id['id']['type'] == 'paces-dispatch-scenario-multi-select')):
                    idx = i
                    break
            ...
            figure[idx] = render_plot(plot_select[idx], data_handler.processed_data['PaCES']['Dispatch'],
        ...
        for i, id in enumerate(ctx.inputs_list[0]):
            if ((id['id']['index'] == trigger_id['index']) and
                    (id['id']['type'] == 'paces-dispatch-plot-select')):
                idx = i
                break
        figure[idx] = render_plot(plot_select[idx], data_handler.processed_data['PaCES']['Dispatch'],
