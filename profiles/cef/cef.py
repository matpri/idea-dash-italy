...
from profiles.cef.callbacks import (emissions as emissions_callbacks,
...
from profiles.cef.processing_scripts import (
...
from profiles.cef.visualization_scripts import (
...
class PyPsaOutput(BaseProfile):
    ...
    def __init__(self):
        super().__init__()
        self.technologies = yaml.load(open('./profiles/cef/technologies.yaml', 'r'), Loader=yaml.FullLoader)
        self.plots = yaml.load(open('./profiles/cef/plots.yaml', 'r'), Loader=yaml.FullLoader)
        self.update_utils()
    ...
    def render_settings(self):
        layout = html.Div(
            [
                ...
                html.Div(id='cef-settings-upload-yaml-output'),
                dmc.Tabs([
                    dmc.TabsList([
                        dmc.Tab('Technology Settings', id='cef-technologies', value='tech'),
                        dmc.Tab('Plot Settings', id='cef-plot-settings', value='plot'),
                    ]
                    ),
                    dmc.TabsPanel(id='cef-technologies-settings', value='tech',
                                  children=self.render_technology_settings()),
                    dmc.TabsPanel(id='cef-plot-settings-panel', value='plot',
    ...
    def render_technology_settings(self):
        ...
        layout = html.Div([
            ...
            html.Div(utils.tech_edit(techs[0]),
                     id='cef-technology-settings-output'),
        ])
