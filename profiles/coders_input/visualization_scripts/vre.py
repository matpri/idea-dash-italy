    ...
    widget_layout = html.Div(
        [
            dmc.Select(
                ...
                id={'type': 'coders_input-vre-variable-dropdown', 'index': window_id},
                data=[{'label': variable, 'value': variable} for variable in variables],
                value=variables[0]
            ),
            dmc.Button('Download Data', id={'type': 'coders_input-vre-download-button', 'index': window_id},
                       variant='light',
                       # center the button
                       style={'display': 'flex', 'justify-content': 'center', 'margin-top': '4px'}),
            dcc.Download(id={'type': 'coders_input-vre-download', 'index': window_id}),
        ],
        style={'textAlign': 'center'})
    plot_layout = dcc.Graph(
        figure=vre_plot(df, variables[0]),
        id={
            'type': ids.FIGURE,
            'index': window_id,
