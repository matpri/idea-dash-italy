...
def plot(df, scenarios, region, aggregate, title, x_axis_label, y_axis_label, season=None):
    fig = go.Figure()
    fig.update_layout(
        title_text=title,
    ...
    try:
        ...
        for i, tech in enumerate(techs):
            ...
            fig.add_bar(x=x, y=data["value"], name=tech, customdata=data['total'], marker_color=color,
                        ...
                        hovertemplate=f'<b>{tech}</b><br><br>' + 'Year: %{x[0]}<br>' + f'Region: {region}<br>' + 'Scenario: %{x[1]}<br>Emissions: %{y:.2f} MtCO2<br>Total: %{customdata:.2f} MtCO2<br><extra></extra>')
