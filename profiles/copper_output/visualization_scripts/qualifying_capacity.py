...
def render_plot(type, df, aggregate, scenarios, region, year, scenario, season, pattern_active=True, text_active=True):
...
def plot(df, window_id):
    ...
    pattern_toggle = dmc.Switch(
        ...
        checked=True,
        id={
            'type': 'copper-qualifying-capacity-pattern-switch',
            'index': window_id,
        },
        style={'display': 'block'}
    )
    ...
    text_toggle = dmc.Switch(
        ...
        checked=True,
        id={
            'type': 'copper-qualifying-capacity-text-switch',
            'index': window_id,
        },
        style={'display': 'block'}
    )
    ...
    widget_layout = html.Div([
        ...
        by_year_widgets,
        by_region_widgets,
        dmc.Button('Download Data', id={'type': 'copper-qualifying-capacity-download-button', 'index': window_id},
                   variant='light',
                   # center the button
                     style={'display': 'flex', 'justify-content': 'center', 'margin-top': '4px'}),
        dcc.Download(id={'type': 'copper-qualifying-capacity-download', 'index': window_id}),
    ])
