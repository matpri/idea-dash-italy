...
def calculate_total_cost(formatted_cost, gen_capacity, fom, vom):
    ...
    carbon_cost = formatted_cost[formatted_cost['variable'] == "Carbon Cost"].copy()
    ...
    carbon_cost = carbon_cost.replace(to_replace="Carbon Cost", value="carbon_tax_cost")
    ...
    fuel_cost_weighted = formatted_cost[formatted_cost['variable'] == "Supply"].copy()
    can_fuel_cost_weighted = fuel_cost_weighted.groupby(["time", "scenario"], as_index=False)["value"].sum(
        numeric_only=True)
    can_fuel_cost_weighted = can_fuel_cost_weighted.assign(region='CAN', variable='Supply')
    ...
    fuel_cost_weighted = fuel_cost_weighted.replace(to_replace="Supply", value="fuel_cost_weighted")

    # Transmission capacity cost == where variable is "Transmission" in formatted df and then add row for Canada as sum of all regions
    transmission_capacity_cost = formatted_cost[formatted_cost['variable'] == "Transmission"].copy()
    can_transmission_capacity_cost = transmission_capacity_cost.groupby(["time", "scenario"], as_index=False).sum(
        numeric_only=True)
    can_transmission_capacity_cost = can_transmission_capacity_cost.assign(region='CAN', variable='Transmission')
    transmission_capacity_cost = pd.concat([transmission_capacity_cost, can_transmission_capacity_cost],
                                           ignore_index=True)
    # Rename values in variable from "Transmission" to transmission_capacity_cost
    transmission_capacity_cost = transmission_capacity_cost.replace(to_replace="Transmission",
                                                                    value="transmission_capacity_cost")

    # Put all costs together in one DataFrame
    total_costs_df = pd.concat([capacity_cost, carbon_cost, fom_cost, fuel_cost_weighted, transmission_capacity_cost,
