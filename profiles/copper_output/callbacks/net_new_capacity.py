...
def link(app):
    @app.callback(
        ...
        Output({
            'type': 'copper-net-new-capacity-year-select',
            'index': ALL
        }, 'style'),
        Output({
        ...
        Output({
            'type': 'copper-net-new-capacity-scenario-select',
            'index': ALL
        }, 'style'),
        Output({
            'type': 'copper-net-new-capacity-scenario-multi-select',
            'index': ALL
        }, 'style'),
        Input({
            'type': 'copper-net-new-capacity-plot-select',
            'index': ALL
        }, 'value'),
        Input({
            'type': 'copper-net-new-capacity-aggregate-switch',
            'index': ALL
        }, 'checked'),
        Input({
            'type': 'copper-net-new-capacity-scenario-multi-select',
        ...
        Input({
            'type': 'copper-net-new-capacity-download-button',
            'index': ALL
        }, 'n_clicks'),
        State({
            'type': 'copper-net-new-capacity-region-select',
    ...
    def update_net_new_capacity(_p_type, _aggregates, _scenarios, _scenario, _regions, _years, _download,_r_style, _y_style, _canvas, _data, _s_style, _m_style):
        print('updating net-new-capacity plot')
        from main import data_handler
        ctx = dash.callback_context
        trigger_id = eval(ctx.triggered[0]['prop_id'].split('.')[0])

        if 'copper-net-new-capacity-download-button' in trigger_id['type']:
            idx = 0
            for i, id in enumerate(ctx.inputs_list[0]):
                if ((id['id']['index'] == trigger_id['index']) and
                        (id['id']['type'] == 'copper-net-new-capacity-download-button')):
                    idx = i
                    break
            _data[idx] = dcc.send_data_frame(data_handler.processed_data['COPPER Output']['Capacity'].to_csv, "net-new-capacity.csv")
            return _canvas, _r_style, _y_style, _data, _s_style, _m_style

        idx = 0
        for i, id in enumerate(ctx.inputs_list[0]):
            if ((id['id']['index'] == trigger_id['index']) and
                    (id['id']['type'] == 'copper-net-new-capacity-plot-select')):
                idx = i
                break
