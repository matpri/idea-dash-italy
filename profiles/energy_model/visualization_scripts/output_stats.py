...
def render_plot(type, df, group_by_model, group_by_scenario, group_by_version, can=True, fill=True):
    ...
    plot_info = plot_settings['Output Stats'][type]
    name = plot_info['name']
    unit = plot_info['unit']
    return plot_output_stats(df, group_by_model, group_by_scenario, group_by_version, plot_info['title'], plot_info['x_label'],
                         plot_info['y_label'], name, unit, fill)


def plot_output_stats(df, group_by_model, group_by_scenario, group_by_version, title, x_label, y_label, name, unit, fill=True):
    fig = go.Figure()
    fig.update_layout(
        title_text=title,
        xaxis_title=x_label,
