...
from profiles.copper_output.processing_scripts import cost_vom as copper_cost_vom
from profiles.nextgrid_output.processing_scripts import cost_vom as nextgrid_cost_vom
...
from profiles.temoa_output.processing_scripts import cost_vom as temoa_cost_vom
...
def check(df):
    ...
    try:
        ...
        elif df.model.unique()[0] == "Sutubra_TEMOA":
            return temoa_cost_vom.check(df)
        else:
            return False
...
def process(selected: dict):
    ...
    for scenario_name, db in selected.items():
        if db.model.unique()[0] == "copper":
            df = copper_cost_vom.process({scenario_name: db})
            dfs.append(df)
            ...
            df = pithos_cost_vom.process({scenario_name: db})
            dfs.append(df)
            ...
            dfs.append(df)
        elif db.model.unique()[0] == "Sutubra_TEMOA":
            df = temoa_cost_vom.process({scenario_name: db})
            dfs.append(df)
        else:
