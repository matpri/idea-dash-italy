...
from profiles.energy_model.visualization_scripts.qualifying_capacity import render_plot
...
def link(app):
    @app.callback(
        ...
        Output({
            'type': 'energy_model-qualifying-capacity-year-select',
            'index': ALL
        }, 'style'),
        Output({
        ...
        Output({
            'type': 'energy_model-qualifying-capacity-scenario-select',
            'index': ALL
        }, 'style'),
        Output({
            'type': 'energy_model-qualifying-capacity-scenario-multi-select',
            'index': ALL
        }, 'style'),
        Output({
        ...
        Output(
            {
                'type': 'energy_model-qualifying-capacity-pattern-switch',
                'index': ALL
            },
            'style'
        ),
        ...
        Input({
            'type': 'energy_model-qualifying-capacity-aggregate-switch',
            'index': ALL
        }, 'checked'),
        Input({
            'type': 'energy_model-qualifying-capacity-scenario-multi-select',
        ...
        Input({
            'type': 'energy_model-qualifying-capacity-download-button',
            'index': ALL
        }, 'n_clicks'),
        Input({
            'type': 'energy_model-qualifying-capacity-season-select',
            'index': ALL
        }, 'value'),
        State({
            'type': 'energy_model-qualifying-capacity-region-select',
    ...
    def update_net_new_capacity(_p_type, _aggregates, _scenarios, _scenario_group, _scenario_version, _scenario, _regions, _report_type, _years, _pattern, _text,_download, _seasons,
                                _r_style, _y_style, _canvas, _data, _s_style, _m_style, _g_style, _v_style, _pattern_style, _text_style, _report_type_style):
        ...
        if 'energy_model-qualifying-capacity-download-button' in trigger_id['type']:
            ...
            for i, id in enumerate(ctx.inputs_list[0]):
                if ((id['id']['index'] == trigger_id['index']) and
                        (id['id']['type'] == 'energy_model-qualifying-capacity-download-button')):
                    idx = i
                    break
            _data[idx] = dcc.send_data_frame(data_handler.processed_data['Power System Models']['Qualifying Capacity'].to_csv,
        ...
        for i, id in enumerate(ctx.inputs_list[0]):
            if ((id['id']['index'] == trigger_id['index']) and
                    (id['id']['type'] == 'energy_model-qualifying-capacity-plot-select')):
                idx = i
                break
        ...
        if trigger_id['type'] == 'energy_model-qualifying-capacity-scenario-group-select':
            ...
            for i, id in enumerate(ctx.inputs_list[0]):
                if ((id['id']['index'] == trigger_id['index']) and
                        (id['id']['type'] == 'energy_model-qualifying-capacity-scenario-group-select')):
                    idx = i
                    break
        ...
        if _p_type[idx] == 'By Year':
            ...
            v_style[idx] = {'display': 'none'}
            if _aggregates[idx] is not None:
                _canvas[idx] = render_plot('Pie Chart', data_handler.processed_data['Power System Models']['Qualifying Capacity'],
                                           _aggregates[idx],
                                           _scenarios[idx],
                                           _regions[idx],
                                           _years[idx], scenario=_scenario[idx])
