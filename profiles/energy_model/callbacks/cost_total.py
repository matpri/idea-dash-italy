...
def link(app):
    @app.callback(
        ...
        Output({
            'type': 'energy_model-total_cost-version-select',
            'index': ALL
        }, 'style'),
        Output({
            'type': 'energy_model-total_cost-version-select',
    ...
    def update_total_cost(_p_type, _compare2ref, _aggregates, _scenarios, _scenario_group, _scenario_version, _scenario, _regions, _report_type, _years, _pattern, _text,
        ...
        if selected_groups and len(selected_groups) > 0 and not (len(selected_groups) == 1 and selected_groups[0] == ''):
            ...
            if trigger_id['type'] == 'energy_model-total_cost-scenario-group-select':
                idx = 0
                for i, id in enumerate(ctx.inputs_list[0]):
                    if ((id['id']['index'] == trigger_id['index']) and
                            (id['id']['type'] == 'energy_model-total_cost-scenario-group-select')):
                        idx = i
                        break
