    ...
    try:
        ...
        for i, tech in enumerate(techs):
            ...
            fig.add_bar(x=x, y=data["value"], name=tech,
                        ...
                        hovertemplate=f'<b>{tech}</b><br><br>' + 'Region: %{x[0]}<br>' + f'Year: {year}<br>' + 'Scenario: %{x[1]}<br>'+f'{tooltip_name}'+': %{y:.2f} '+f'{unit}'+'<br>Total: %{customdata:.2f}' + f'{unit}<br>' +
...
def subset(df, year, scenarios, aggregate, season=None):
    ...
    def get_sum(row, col_name):
        return df_scen.loc[
            (df_scen['region'] == row['region']) & (df_scen['scenario'] == row['scenario']), col_name].sum()
